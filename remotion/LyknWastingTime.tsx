...
import { LyknStillWastingTime, STILL_WASTING_TIME_DURATION } from "./LyknStillWastingTime";

// ---------------------------------------------------------------------------
// The full reel. Opens with the "Still wasting time / switching tabs?" beat
// (its own comp — it ends inverted on black), then the existing
// "{ Say hello to }" sequence plays out on the black background, followed by
// the chat bar, invisible mode, the closing words and the icon lockup.
// ---------------------------------------------------------------------------

const FONT = "Inter, system-ui, sans-serif";

// ── timeline (30 fps) ──
// The opener runs on its own clock; everything else is chained after it.
const HELLO_FROM = STILL_WASTING_TIME_DURATION;
...
export const LyknWastingTime: React.FC = () => {
  return (
    // the opener paints its own white background and hands off on black
    <AbsoluteFill style={{ background: "#000000", fontFamily: FONT, overflow: "hidden" }}>
      {/* "Still wasting time / switching tabs?" → inverts on the "?" click */}
      <Sequence from={0} durationInFrames={STILL_WASTING_TIME_DURATION}>
