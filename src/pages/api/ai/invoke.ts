...
`;
    ...
    const buildLyknChatPrompt = (input: {
      ...
      const systemPrompt = [
        ...
        "",
        "Users may raise one or more blocks (e.g. by double-pressing). Raised/focused blocks are marked [FOCUSED] in the Grid context.",
        "When focused blocks exist: the user's prompt refers specifically to those raised brick(s). Interpret and answer only in relation to those brick(s) (e.g. that image, that note) unless they clearly ask about something else.",
    ...
    if (wantsActions) {
      ...
      if (userIntent === "mindmap") {
        ...
        prompt = [
          ...
          "- Use update_text_block when the user asks to edit, change, rewrite, add to, or reorganize text in an existing brick. Modify in place — do NOT delete and recreate.",
          "- When the user asks to add items to a list, use the append field to add lines rather than replacing all content.",
          ...
          "",
          "UPDATE TEXT BRICK (edit content and/or format of an existing text brick on the Grid):",
          ...
          '- { "type": "update_text_block", "blockId": "id-from-context", "data": { "textVariant": "h1" } }',
          '- { "type": "update_text_block", "blockId": "id-from-context", "content": "rewritten content", "data": { "listType": "todo" } }',
          "  Modifies an existing text brick in place. The block ID comes from the Grid context (the id= field).",
          "  - blockId (REQUIRED): the id of the text brick to update.",
          "  - content (optional): replaces the block's entire text content. Omit to keep existing content.",
          "  - append (optional): text to append to the end of the block's existing content. Useful for adding items to a list without rewriting everything. Ignored if content is also provided.",
          "  - data.textVariant (optional): change to \"body\", \"h1\", or \"h2\".",
          "  - data.listType (optional): change to \"none\", \"bullet\", \"numbered\", \"todo\", \"toggle\", or \"quote\".",
          "  - data.brickColor / data.textColor (optional): change brick or text color.",
          "  When to use:",
          "  - User asks to edit, change, update, fix, rewrite, or modify text in a specific brick.",
          "  - User asks to add items to or remove items from an existing list or checklist.",
          "  - User asks to reorganize, reformat, or restructure existing text.",
          "  - User asks to change a heading level, list type, or brick style.",
          "  IMPORTANT: Prefer update_text_block over delete + create when the user wants to modify existing content.",
          "",
