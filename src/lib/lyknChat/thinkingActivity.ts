/**
 * Timeline language for the chat thinking row: map status lines to a kind,
 * collapse consecutive same-kind work into "Ran 3 searches", and format the
 * live header ("Working for 6s"). Presentation lives in ThinkingIndicator.
 */
...
const AGGREGATE_KINDS = new Set<ThinkingActivityKind>(["search", "read", "image"]);
...
export function stripStatusDecor(line: string): string {
  return String(line || "")
    .trim()
    .replace(/[.…]+$/g, "")
    .trim();
...
/** Collapse progress ticks ("Reading sources (2/5)", "Writing the code… (12k)") into one family. */
export function trailFamilyKey(status: string): string {
...
export function activityKindFromLine(line: string): ThinkingActivityKind {
...
export function formatAggregate(kind: ThinkingActivityKind, count: number): string {
...
export function formatWorkingFor(seconds: number): string {
...
export function thinkingHeaderLabel({
  ...
  if (stepCount > 0) return formatWorkingFor(elapsedSeconds);
  return String(fallback || "").trim() || "Thinking…";
}

const MAX_STEPS = 6;

/**
 * Turn a status trail + live line into timeline rows.
 * Consecutive search/read/image bursts collapse. The live specific line
 * stays present-tense at the bottom and is not counted in the burst above it.
 */
export function collapseThinkingSteps(trail: string[] | undefined, live: string): ThinkingStep[] {
  const liveTrim = String(live || "").trim();
  const completed: Array<ThinkingStep & { count: number }> = [];
  ...
  const steps: ThinkingStep[] = completed.map(({ kind, label }) => ({ kind, label }));

  if (liveTrim) {
    const kind = activityKindFromLine(liveTrim);
    steps.push({
      kind,
