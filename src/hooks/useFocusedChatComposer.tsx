...
export function useFocusedChatComposer({
  ...
  const persistSelectedModel = useCallback(
    (value: string) => {
      const allowed = isAgentStudio
        ? isAgentStudioModelAllowed(value, modelTier)
  ...
  useEffect(() => {
    ...
    const allowed = isAgentStudio
      ? isAgentStudioModelAllowed(selectedModel, modelTier)
      : isModelAllowedForPlan(selectedModel, modelTier);
    if (allowed) return;
    const fallback = isAgentStudio
      ? defaultAgentBuilderModelForPlan(modelTier, { devUnlock: isAgentStudioEnabled })
