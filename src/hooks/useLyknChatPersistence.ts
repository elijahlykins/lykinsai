...
export function useLyknChatPersistence(params: UseLyknChatPersistenceParams) {
  ...
  const [title, setTitle] = useState(() => {
    try { return localStorage.getItem("lyknchat_title") || ""; }
    catch { return ""; }
  });
  ...
  /** True once this board has a row in `lykn_chats` (loaded or first save). */
  ...
  /* ------------------------------------------------------------------ */
  useEffect(() => {
    try { localStorage.setItem("lyknchat_title", title); } catch { /* ignore */ }
  }, [title]);
  ...
  const applySnapshot = useCallback(
    (snapshot: any, hydrateChatId?: string | null) => {
      ...
      if (chatChatId) {
        ...
        restoreSavedToVaultState(chatChatId);
      }
  ...
  /*  and was never saved before, we delete the stale lykn_chats row   */
  /*  that was eagerly created on load so it never appears in the UI.   */
  /* ------------------------------------------------------------------ */
  const saveSnapshot = useCallback(async (opts?: { isFinal?: boolean }) => {
    if (!userId || !chatId || savingRef.current || !hydratedRef.current) return;
    if (isDemoLyknChatId(chatId)) return; // demo grids are read/edit-only; never hit the DB
    ...
    if (neverSavedBefore && titleUntouched && isBoardEmpty()) {
      if (opts?.isFinal) {
        ...
        try {
          ...
          try { localStorage.removeItem(`lyknchat_chat_${chatId}`); } catch { /* ignore */ }
          try { localStorage.removeItem(`lyknchat_camera_${chatId}`); } catch { /* ignore */ }
          try { localStorage.removeItem(`lyknchat_vault_saved_${chatId}`); } catch { /* ignore */ }
          try {
    ...
    try {
      ...
      if (!boardRowExistsRef.current) {
        ...
        try { localStorage.setItem("lyknchat_active_id", chatId); } catch { /* ignore */ }
      }

      const statePayload = { chat_id: chatId, state: snapshot, version: raw.version || SNAPSHOT_VERSION, user_id: userId, updated_at: now };
      ...
      const boardStateUpsert = supabase
        ...
        .upsert(statePayload, { onConflict: "chat_id" })
        .select("chat_id, user_id, updated_at");

      let updateRes = await supabase.from("lykn_chats").update(boardUpdatePayload).eq("id", chatId);
      if (
        updateRes.error &&
        chatModelKey &&
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [chatId, buildSnapshot, isBoardEmpty, userId]);
  ...
  useEffect(() => {
    ...
    if (routeChatId && isDemoLyknChatId(routeChatId)) {
      (async () => {
        ...
        const snapshot = getDemoLyknChatSnapshot(routeChatId);
        if (cancelled) return;
    ...
    // and navigates to `/chat/:id`. Hydrating here mints a second UUID;
    ...
    const loadBoard = async () => {
      ...
      const priorChatId = chatIdRef.current;
      ...
      try {
        const existing = routeChatId || localStorage.getItem("lyknchat_active_id");
        if (existing) {
          ...
          if (data?.id) {
            ...
            localStorage.setItem("lyknchat_active_id", id!);
          }
        }
      } catch {
        // ignore
      ...
      if (!id && routeChatId) {
        ...
        try {
          if (localStorage.getItem("lyknchat_active_id") === routeChatId) {
            localStorage.removeItem("lyknchat_active_id");
          }
        } catch {
          // ignore
      ...
      if (!id && !routeChatId) {
        try {
          const recent = await fetchMostRecentLyknChat(userId);
          if (recent?.id) {
            id = recent.id;
            boardRowExistsRef.current = true;
      ...
      const isBoardSwitch = Boolean(priorChatId && priorChatId !== id);
      const shouldRestorePendingChat = !isExplicitNewChat && !isBoardSwitch;
      // Restore an in-memory thread snapshot for `chatId` over the DB load
      ...
      const restorePreferredRuntimeChat = (chatId: string, loadedChat: any[]) => {
        try {
          const rtSnap = getThreadSnapshot(chatId);
          if (!rtSnap) return;
      ...
      try {
        let draft: any = null;
        try {
          const raw = localStorage.getItem(`lyknchat_draft_${id}`);
        ...
        if (useDraft) {
          ...
          try {
            const lsCam = localStorage.getItem(`lyknchat_camera_${id}`);
            if (lsCam) {
        ...
        try {
          localStorage.removeItem(`lyknchat_draft_${id}`);
          localStorage.removeItem(`lyknchat_chat_${id}`);
          localStorage.removeItem(`lyknchat_camera_${id}`);
        } catch { /* ignore */ }
        try {
          applySnapshotRef.current({
            version: SNAPSHOT_VERSION,
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(`lyknchat_vault_saved_${chatId}`, JSON.stringify({
        mediaUrls: [...savedMediaUrls],
        youtubeIds: [...savedYouTubeIds],
      }));
    } catch { /* quota */ }
  }, [chatId, savedMediaUrls, savedYouTubeIds]);
  ...
  useEffect(() => {
    if (!chatId || !userId) return;
    if (isDemoLyknChatId(chatId)) return; // demo grids never autosave
    ...
    window.addEventListener("lyknchat_flush_save", onFlushSave);
    document.addEventListener("visibilitychange", onVisibilityChange);
    return () => {
      window.clearInterval(autoSaveInterval);
      window.removeEventListener("beforeunload", onBeforeUnload);
      window.removeEventListener("pagehide", onPageHide);
      window.removeEventListener("lyknchat_flush_save", onFlushSave);
  ...
  /* ------------------------------------------------------------------ */
  useEffect(() => {
    try { localStorage.removeItem("lyknchat_seed_v2"); } catch { /* ignore */ }
  }, []);
