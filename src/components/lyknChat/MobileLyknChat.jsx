 ...
 */
export default function MobileLyknChat() {
  ...
  const { data: boards = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!user?.id) return [];
      return fetchLyknChatsWithContext(user.id, 100);
    },
    enabled: !!user?.id,
  });
  ...
  const list = useMemo(() => {
    if (!user?.id) return [];
    return mergeActiveRouteLyknChat(boards, location.pathname);
  ...
  const goToGrid = (id) => {
    setOpen(false);
    if (!user?.id) return;
    if (location.pathname === `/chat/${id}`) return;
    addOpenThread(id);
    flushAndNavigate(nav, `/chat/${id}`);
  };
  ...
  const createNewGrid = async () => {
    ...
    try {
      ...
      flushAndNavigate(nav, `/chat/${chatId}`);
    } catch {
      /* ignore */
    }
  };
            ...
            <div className="flex-1 min-h-0 overflow-y-auto px-2 pb-3">
              {filtered.length === 0 ? (
                ...
                <ul className="flex flex-col">
                  {filtered.map((b) => {
                    const isActive =
                      String(routeChatId || "") === String(b.id) ||
                      location.pathname === `/chat/${b.id}`;
                    const canEdit = !!user && !isDemoLyknChatId(b.id);
                    return (
