      ...
      <div className="relative z-[1] w-full max-w-3xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between gap-2 mb-2">
          {hasDetails ? (
            ...
            <span className="text-[0.78rem] font-medium text-white/80 px-1 max-w-[16rem] truncate">{title}</span>
          )}
          <button
            type="button"
            onClick={onClose}
        ...
        {detailsOpen && hasDetails ? (
          <div className="mb-2 rounded-2xl border border-black/10 dark:border-white/12 bg-panel backdrop-blur-xl shadow-none px-4 py-3.5 max-h-[42vh] overflow-y-auto space-y-3">
            {noteId ? (
              <button
                type="button"
                onClick={openInVault}
        ...
        <div className="flex-1 min-h-0 overflow-y-auto rounded-2xl">
          ...
          {hasAttachments ? (
            ...
            <div className="rounded-2xl border border-black/10 dark:border-white/12 bg-panel backdrop-blur-xl shadow-none px-5 sm:px-6 py-5">
              {bodyMarkdown}
            </div>
          ) : !loading ? (
