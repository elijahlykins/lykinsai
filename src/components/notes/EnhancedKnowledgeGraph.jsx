...
export default function EnhancedKnowledgeGraph({ 
  ...
  const buildGraph = async () => {
    ...
    try {
      ...
      const prompt = `Analyze these notes and identify semantic relationships between them. For each pair of related notes, provide:
...
Return ONLY a JSON object: {"relationships": [...], "clusters": [...]}`;
      ...
      let analysis = { relationships: [], clusters: [] };
      try {
...
Return ONLY a JSON object: {"suggestions": [{"note_id": "id", "reason": "reason", "type": "type"}, ...]}`;
      ...
      try {
        suggestionResults = JSON.parse(aiText);
      } catch (e) {
        const jsonMatch = aiText.match(/\{[\s\S]*\}/);
        if (jsonMatch) {
          try {
            suggestionResults = JSON.parse(jsonMatch[0]);
          } catch {}
        }
      }
    ...
    try {
      ...
      if (fromNote && toNote) {
        ...
        if (onUpdateConnections) {
          onUpdateConnections(fromNote.id, fromConnections);
          onUpdateConnections(toNote.id, toConnections);
        }
