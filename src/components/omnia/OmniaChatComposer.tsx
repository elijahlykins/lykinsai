...
export function resizeOmniaChatInput(el: HTMLTextAreaElement | null) {
  if (!el) return;
  const maxH = 180;
...
export type OmniaChatComposerProps = {
  value: string;
...
/**
 * Shared neumorphic chat composer shell — matches OmniaGrid / OmniaCenterWelcome.
 */
const OmniaChatComposer = React.memo(function OmniaChatComposer({
  value,
  onChange,
  onSend,
  ...
  const defaultToolbar = !hideDefaultSend ? (
    <div className={`flex items-center gap-1.5 ${compact ? "pt-0.5" : "pt-1"}`}>
      ...
      <button
        ...
        className={`${iconBtn} omnia-neu-chat-send-btn flex items-center justify-center shrink-0 ${
...
export default OmniaChatComposer;
