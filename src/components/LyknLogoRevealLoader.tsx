// Loading animation that mirrors the Remotion `LyknLogoReveal`: the icon
// traces its outline on, fills to solid, then the wordmark wipes open while
// the icon glides left to center the finished lockup. Loops for long loads.
// Animation lives in index.css (`.lykn-reveal-loader`).
...
// Left edge cropped to where the "L" begins so the wordmark sits tight.
const WORDMARK_VIEWBOX = "38.92 0 441.35 194.53";
const WORDMARK_ASPECT = 441.35 / 194.53;
const WORDMARK_PATH =
...
interface LyknLogoRevealLoaderProps {
  /** Icon height in px (the wordmark scales relative to this). */
  size?: number;
  className?: string;
}

export default function LyknLogoRevealLoader({
  size = 88,
  className = "",
}: LyknLogoRevealLoaderProps) {
  const wordH = size * 0.82;
  const wordW = wordH * WORDMARK_ASPECT;
  ...
  return (
    <div
      className={`lykn-reveal-loader ${className}`}
      style={wrapperStyle}
      role="img"
      aria-label="Loading LYKN"
    >
      <svg
        className="lykn-reveal-loader__icon"
        width={size}
        height={size}
      ...
      <div className="lykn-reveal-loader__word">
        <svg width={wordW} height={wordH} viewBox={WORDMARK_VIEWBOX} fill="none">
