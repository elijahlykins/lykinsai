  ...
  const runMessage = useCallback(
    async (text) => {
      ...
      try {
        ...
        const reply = String(data.result?.reply || "No reply.").trim();
        const runtime = data.result?.runtime || "llm";
        setHasRun(true);
        setWorkspaceOutput(reply);
        setWorkspaceRuntime(runtime);
        setWorkspaceToolCalls(Array.isArray(data.result?.tool_calls) ? data.result.tool_calls : []);
        setWorkspaceWarning(String(data.result?.handler_warning || ""));
        setMessages((prev) => [
          ...prev,
