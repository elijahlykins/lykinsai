...
// ────────────────────────────────────────────────────────────────────────
// LyknTodosDialog — the to-do list pop-up.
//
// Reads/writes the user's own lykn_todos rows directly through the
// RLS-protected Supabase client (same pattern LyknCalendarDialog uses for
// events), and subscribes to realtime so tasks the AI adds / completes in
// text or voice appear here live. A task has an open → completed | cancelled
// lifecycle and an OPTIONAL due date.
// ────────────────────────────────────────────────────────────────────────
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[calc(100vw-1.5rem)] max-w-lg max-h-[88dvh] overflow-hidden flex flex-col sm:w-full [color-scheme:dark]">
        ...
        {/* List */}
        <div className="flex-1 overflow-y-auto -mx-2 px-2 py-1 min-h-[8rem]">
          {loading ? (
