...
import { useSignupGrantNotice } from "@/hooks/useSignupGrantNotice";

function GlassNotice({ title, summary, children }) {
  return (
    <div className="flex items-start gap-3 rounded-2xl border border-white/20 bg-black/55 px-3.5 py-2.5 text-white shadow-[0_12px_32px_rgba(0,0,0,0.28)] backdrop-blur-xl">
      <div className="min-w-0 flex-1">
        ...
        {summary ? (
          <p className="mt-0.5 text-[11px] leading-snug text-white/65">{summary}</p>
        ) : null}
      </div>
      {children}
    </div>
...
export default function StudioUpdateBanners({ onOpenAccount }) {
  const credits = useSignupGrantNotice();
  ...
  const showCredits = credits.visible;
  ...
  return (
    <div className="pointer-events-none absolute inset-x-0 top-3 z-[60] flex justify-center px-4">
      <div className="pointer-events-auto flex w-full max-w-[420px] flex-col gap-2">
        ...
        {showProduct ? (
          <GlassNotice title={product.update.title} summary={product.update.summary}>
