// Full LYKN logo (icon + wordmark) loading animation: draws the outline on,
// crossfades to the solid logo, holds, then fades out and loops. Same
// stroke-draw effect as the chat LyknOutlineSpinner, applied to the whole
// lockup. Animation lives in index.css (`.lykn-logo-loader`).

const ICON_VIEWBOX = "0 0 204.29 204.29";
const ICON_PATH =
  "M167.39,60.26l-.86-.39c-9.83-4.41-17.7-12.28-22.12-22.12l-.39-.86c-1.77-3.94-7.36-3.94-9.13,0l-.39.86c-4.41,9.83-12.28,17.71-22.12,22.12l-.86.39c-3.94,1.77-3.94,7.36,0,9.13l.86.39c9.83,4.41,17.7,12.28,22.12,22.12l.39.86c1.77,3.94,7.36,3.94,9.13,0l.39-.86c4.41-9.83,12.28-17.7,22.12-22.12l.86-.39c3.94-1.77,3.94-7.36,0-9.13ZM134.87,116.05c-14.73,2.8-17.97,18.72-32.73,18.72-8.11,0-12.75-4.81-17.72-9.61-1.8-1.73-3.56-3.5-5.29-5.29-4.8-4.98-9.62-9.61-9.62-17.73,0-14.76,15.93-18,18.72-32.73,2.66-14.03-7.74-27.55-21.99-28.38-13.8-.8-25.24,10.16-25.24,23.79,0,18.8,19.14,21.14,19.14,37.32s-19.14,18.52-19.14,37.32c0,13.16,10.67,23.83,23.83,23.83,18.8,0,21.14-19.14,37.32-19.14s18.52,19.14,37.32,19.14c13.63,0,24.58-11.44,23.78-25.24-.82-14.25-14.35-24.66-28.38-21.99Z";

// Left edge cropped to the first glyph (the "L" starts ~x=38.92) so the
// wordmark sits tight against the icon without built-in whitespace.
const WORDMARK_VIEWBOX = "38.92 0 441.35 194.53";
const WORDMARK_ASPECT = 441.35 / 194.53;
const WORDMARK_PATH =
  "M113.42,136.31h-39.19c-2.23,0-4.05-1.82-4.05-4.05V39.72c0-.45-.36-.81-.81-.81h-29.64c-.45,0-.81.36-.81.81v86.71c0,20.57,11.2,29.2,27.65,29.2h46.86c.45,0,.81-.36.81-.81v-17.69c0-.45-.36-.81-.81-.81ZM403.29,36.25c-15.94,0-26.62,7.59-32.83,15.64-1.48,1.92-2.75,1.47-2.75-.96v-11.21c0-.45-.36-.81-.81-.81h-27.94c-.45,0-.81.36-.81.81v115.09c0,.45.36.81.81.81h29.57c.45,0,.81-.36.81-.81v-62.28c0-24.75,10.56-34.37,22.16-34.37s18.66,7.86,18.66,22.14v74.51c0,.45.36.81.81.81h29.57c.45,0,.81-.36.81-.81v-79.75c0-25.52-16.91-38.81-38.08-38.81ZM285.84,84.42c-.54-.9-.4-2.28.31-3.05l37.57-41.1c.48-.52.11-1.36-.6-1.36h-26.05c-.48,0-.93.21-1.24.57l-43.12,50.89c-1.5,1.77-2.72,1.32-2.72-1v-49.65c0-.45-.36-.81-.81-.81h-29.64c-.45,0-.81.36-.81.81v115.09c0,.45.36.81.81.81h29.64c.45,0,.81-.36.81-.81v-31.5c0-1.32.73-3.19,1.61-4.16l10.7-11.72c.79-.86,1.91-.74,2.48.28l26.64,47.09c.29.51.83.82,1.41.82h34.13c.63,0,1.02-.69.7-1.23l-41.83-69.98ZM210.32,38.91h-23.32c-.61,0-1.17.34-1.45.89l-23.86,47.1c-.77,1.51-2.01,1.51-2.76-.01l-23.23-47.07c-.27-.55-.84-.91-1.46-.91h-33.97c-.62,0-1.01.66-.71,1.2l39.62,72.63v42.08c0,.45.36.81.81.81h29.64c.45,0,.81-.36.81-.81v-42.4l40.57-72.29c.3-.54-.09-1.21-.71-1.21Z";

interface LyknLogoLoaderProps {
  /** Icon height in px (the wordmark scales relative to this). */
  size?: number;
  className?: string;
}

export default function LyknLogoLoader({
  size = 84,
  className = "",
}: LyknLogoLoaderProps) {
  const wordH = size * 0.82;
  const wordW = wordH * WORDMARK_ASPECT;
  const gap = Math.round(size * 0.06);

  const pathProps = {
    pathLength: 1,
    fill: "currentColor",
    stroke: "currentColor",
    strokeWidth: 1.75,
    strokeLinecap: "round" as const,
    strokeLinejoin: "round" as const,
    vectorEffect: "non-scaling-stroke" as const,
  };

  return (
    <div
      className={`lykn-logo-loader ${className}`}
      style={{ gap }}
      role="img"
      aria-label="Loading LYKN"
    >
      <svg width={size} height={size} viewBox={ICON_VIEWBOX} fill="none">
        <path d={ICON_PATH} {...pathProps} />
      </svg>
      <svg
        className="lykn-logo-loader__word"
        width={wordW}
        height={wordH}
        viewBox={WORDMARK_VIEWBOX}
        fill="none"
      >
        <path d={WORDMARK_PATH} {...pathProps} />
      </svg>
    </div>
  );
}
