...
test("identical bursts dedupe inside the window", () => {
  ...
  const input = { routineId: "r", title: "Routine: pricing" };
...
test("clicking the native notification routes the deep link", () => {
  const { service, sent } = makeService();
  service.notify({ routineId: "r-1", title: "Routine needs you" });
  sent.native[0].click();
