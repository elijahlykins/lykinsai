...
/**
 * Durable persistence for Routines — definitions, run history, and
 * minimal monitor state. Routines are user-owned standing work; they belong
 * to no other entity.
 *
 * PERSISTENCE DECISION (documented per the Phase 4 brief): the renderer
 * cannot fire anything while no window is open, so Routines need a
 * MAIN-process durable source of truth. The two existing conventions are
 * (a) JSON files in userData (local-mode.json, overlay-agents.json) and
 * (b) the SQLite localStore, whose lifecycle is coupled to the Vault store
 * being configured. Routines are small structured records (definitions +
 * bounded history), so this store follows convention (a): one JSON file,
 * atomic tmp+rename writes, debounced persistence with an explicit
 * persistNow for pre-fire checkpoints. That is the simplest storage that
 * survives restart and has no coupling to Local Mode being enabled.
 *
 * What is durable:
 *   - Routine definitions
...
const STORE_FILE = "routines.json";
/** Pre-elimination store name (routines used to belong to Bots). Read-only
 *  migration source: loaded once when the new file does not exist yet. */
const LEGACY_STORE_FILE = "bot-routines.json";
...
function createRoutineStore({ userDataPath, now = () => Date.now(), onChange = () => {} } = {}) {
  ...
  function load() {
    ...
    try {
      ...
      // One-time migration: routines used to be stored per-Bot in
      // bot-routines.json. Read it once; the next persist writes the new file
      // (the legacy file is left in place, never written again).
      try {
        raw = JSON.parse(fs.readFileSync(path.join(path.dirname(file), LEGACY_STORE_FILE), "utf8"));
    ...
    for (const routine of Array.isArray(raw.routines) ? raw.routines : []) {
      ...
      // Routines are user-owned: drop the retired Bot association fields.
      delete routine.botId;
      delete routine.bot;
    ...
    if (migratedFromLegacy && routines.size) persistNowSync();
