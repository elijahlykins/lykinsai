 ...
 * AgentExecutor (electron/agentRuntime.cjs runRoutineOccurrence). TaskRuntime
...
function createRoutineRuntime({
  ...
  function notifyOutcome(routine, runId, outcome, triggerContext) {
    ...
    if (waiting) {
      ...
      result =
        routine.notificationPolicy === "silent"
          ...
          : notifications.notify({
              ...
              title: `LYKN needs you: ${routine.name}`,
      ...
      result = notifications.notify({
        ...
        title: failed ? `${routine.name} failed` : routine.name,
   ...
   */
  function createRoutineFromInstruction(instruction, { notificationPolicy, browserContext, windowContext, connectionIds, inboxIdentity } = {}) {
  ...
  function listRoutines() {
    return store.list().map(publicRoutine);
  }
