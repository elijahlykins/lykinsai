 ...
 * terminal delivery.
 *
 * Run: node --test electron/agent-harness/agentHarness.test.cjs
 */
...
test("system prompt carries identity + rules + tool index, but no full tool docs", () => {
  const system = contextRouter.buildDecisionSystem({ localMode: false });
  // Corpus layers.
...
test("system prompt contains no em dashes", () => {
  const system = contextRouter.buildDecisionSystem({ localMode: true });
...
test("system prompt is byte-stable, and local mode gates the local tool", () => {
  const a = contextRouter.buildDecisionSystem({ localMode: false });
  const b = contextRouter.buildDecisionSystem({ localMode: false });
  assert.equal(a, b);
  assert.doesNotMatch(a, /`local_computer`/);
  assert.match(a, /`ai_drive`/);
  assert.match(a, /`create_routine`/);
  assert.match(a, /`connected_apps`/);
  const withLocal = contextRouter.buildDecisionSystem({ localMode: true });
...
test("first selection of a tool loads its doc instead of running; the call runs next round", async () => {
  ...
  const res = await harness.runAgentTask({
...
test("delivering with nothing run gets one pushback, then stands", async () => {
  ...
  const res = await harness.runAgentTask({ goal: "task", model, executors: {} });
  assert.equal(res.status, "completed");
...
test("ask_user hands back one bundled question with tappable options", async () => {
  ...
  const res = await harness.runAgentTask({ goal: "email dana", model, executors: {} });
...
test("an unknown tool is noted and the loop keeps going", async () => {
  ...
  const res = await harness.runAgentTask({ goal: "x", model, executors: {} });
  assert.equal(res.status, "completed");
...
test("every round carries a full brief — TASK / SUCCESS CONDITION / SCOPE / DO NOT / STOP RULE", async () => {
  const { model, seen } = fakeModel([{ kind: "deliver", answer: "hi" }, { kind: "deliver", answer: "hi" }]);
  await harness.runAgentTask({ goal: "check the user's email", model, executors: {} });
