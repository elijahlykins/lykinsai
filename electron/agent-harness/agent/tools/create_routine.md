...
Set up a routine: a task LYKN will run on its own, on a schedule or when
...
occurrence later runs as its own task and notifies the user per its
notification policy.
...
- Only create a routine the user actually asked to be recurring or standing.
  ...
  A task they taught you by demonstration is a saved workflow, not a new
...
- Do not create duplicates: if the user refines an existing routine, say the
  routine already exists and what it does.
