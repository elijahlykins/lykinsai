...
/**
 * Tools an agent may run without a planning round in front of them.
 *
...
class AgentExecutor {
  constructor({ runAgentTask } = {}) {
    if (typeof runAgentTask !== "function") throw new TypeError("runAgentTask is required");
  ...
  async execute(task, runtime = {}) {
    ...
    // ── Direct execution ────────────────────────────────────────────────
    //
    // Casual agent chat has always taken this branch: one
    ...
    try {
      ...
      if (runtime.signal?.aborted) {
        return { ok: false, status: "cancelled", output: "", executor: `agent-${primaryTool || "reply"}-fallback` };
      }
      if (fallbackResult?.status === "waiting_for_approval") {
        return {
          ...
          executor: `agent-${primaryTool || "reply"}-fallback`,
        };
      }
...
module.exports = { AgentExecutor, resolveDirectTool, looksMultiPart, DIRECT_TOOLS};
