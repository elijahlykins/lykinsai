...
function cleanConnectionIds(value) {
  ...
  return [...new Set((Array.isArray(value) ? value : [value])
    ...
    .filter((id) =>
      id.length > 0 &&
      id.length <= 80 &&
      !/token|secret|bearer|password/i.test(id) &&
      !id.includes(".") &&
      /^[a-zA-Z0-9_-]+$/.test(id),
    ))].slice(0, 20);
}
...
function validateWorkflowDefinition(input) {
  ...
  for (const connection of input.connections || []) {
    ...
    const cleanId = cleanConnectionIds([connection.id])?.[0];
    if (!["mcp", "remote"].includes(connection.kind) || cleanId !== connection.id) {
      throw new TypeError("Invalid workflow connection");
    }
    connectionKeys.add(`${connection.kind}:${connection.id}`);
  }
  ...
  for (const step of input.steps) {
    ...
    if (step.kind === "task" && step.human_takeover !== true) {
      throw new TypeError("Task workflow steps must be explicit human takeover boundaries");
    }
    if (step.kind === "mcp" && !connectionKeys.has(`mcp:${step.target?.connectionId}`)) {
      throw new TypeError("MCP workflow step must reference a declared connection");
    }
    if (step.kind === "remote" && !connectionKeys.has(`remote:${step.target?.remoteTargetId}`)) {
      throw new TypeError("Remote workflow step must reference a declared target");
    }
  ...
  for (const event of events) {
    ...
    if (event.kind === "local") {
      if (event.metadata?.sourceDomain && event.metadata.sourceDomain !== "local") {
        ...
        result.add(write
          ? (/execute|run|shell/i.test(event.action) ? "local.shell.execute" : "files.write")
          : "files.read");
      }
    }
