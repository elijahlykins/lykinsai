...
class WorkflowExecutor {
  ...
  async runSteps(definition, parameters, task, runtime, options) {
    ...
    let recoveries = 0;
    const approvedSteps = new Set(Array.isArray(options.approvedSteps) ? options.approvedSteps : []);
    for (const sourceStep of definition.steps) {
      ...
      if (
        ...
        const request = {
          ...
          stepId: step.id,
          action: step.action,
          question: `Approve ${step.action} for step ${step.id}?`,
        };
        runtime.approvalRequired?.(request);
        if (typeof options.requestApproval !== "function") {
          return { status: "waiting_for_approval", question: request.question, completed };
        }
        const approved = await options.requestApproval(request).catch(() => false);
        runtime.approvalResolved?.(approved);
        if (!approved) {
          return {
            status: "waiting_for_user",
            question: `Step ${step.id} was not approved.`,
      ...
      while (true) {
        const childTask = {
          ...
          approval: {
            ...task.approval,
            state: approvedSteps.has(step.id) ? "approved" : "not_requested",
          },
          association: {
            ...task.association,
