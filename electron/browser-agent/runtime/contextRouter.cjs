...
function decisionOutputContract(allowedActions = null) {
  ...
  const actionGuidance = [
    ...
    has("paste_text") &&
      '  - `paste_text`: put a WHOLE document body into the editor on this page, in one go - how you write a long piece into Notion, Docs or Slides. It finds and focuses the editor itself, so DO NOT hunt for the writing area first: paste, then read the page back to confirm. Do not click Rename / Untitled document afterwards unless they asked you to name the file. Typing a long document instead is slow and lets autocomplete and autosave interfere. Set `mode` to "replace" to overwrite what is there.',
    has("screenshot") &&
  ...
  return [
    ...
    ...actionGuidance,
    '- kind "finish": every part of the goal is done with evidence, or it is genuinely impossible; `answer` is the final report the user keeps. After an action, say what you did and what they now have. After findings (inbox, a listing, a comparison, research), write a real markdown report: title, short summary, headed sections, lists or a table, and sources or links when you have them. Longer is better than a teaser. Do NOT finish with plan steps still outstanding unless you say in `answer` why each one no longer applies. A goal that asks for several deliveries ("email Alice and text Bob") is finished only when every one of them has happened. If the page already shows the outcome - "Message sent", "Access updated", the compose window gone after a send - you are done: finish now. Do not compose again, do not click Send again, do not start the task over. And the moment the goal is met, finish - one more look around, one more page, one more check is browsing past the end of the task.',
