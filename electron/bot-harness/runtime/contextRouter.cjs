 ...
 *
 * Every decide round used to run against BOT_DECISION_SCHEMA with a 4,000
 ...
 * BOT_DELIVER_SCHEMA, which is the only one that needs the room.
 *
 * `kind` keeps "deliver" so the model can still SAY it is finished; the loop
 * then makes the deliver call to get the words.
 */
const BOT_STEP_SCHEMA = {
  type: "object",
  properties: Object.fromEntries(
    Object.entries(BOT_DECISION_SCHEMA.properties).filter(([k]) => k !== "answer"),
  ),
  required: ["kind"],
  additionalProperties: false,
};
...
const BOT_DELIVER_SCHEMA = {
  type: "object",
  properties: {
    answer: BOT_DECISION_SCHEMA.properties.answer,
  },
  required: ["answer"],
  additionalProperties: false,
};
...
function buildTaskUser({
  ...
  // message, not the system prompt: the system prompt is byte-stable per bot
  // so its cache prefix survives, and this list is per-user and changes.
