 ...
 * build the campaign" apart from "what is Mailchimp good for?" needs the venue
 * table and the skill classifier, which live in the main process. When nobody
 ...
 * 2. The user named where — a known product, or a real URL.
 *
...
function decideChatRoute(text) {
  ...
  let url = "";
  try {
    venue = String(ownedBrowserAct.resolveNamedWorkVenueUrl?.(prompt) || "");
  ...
  return {
    route: "agent",
    skill,
    // A named product only. This is what the user gets told, and a
