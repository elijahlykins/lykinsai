...
//
// Since the server decomposition (Wave 1+), route handlers live in both
// server.js and server/routes/*.routes.js, so the scan covers the
// concatenation — the guards follow the code wherever it is registered.
...
const serverSrc = (
  await Promise.all(
    [join(root, 'server.js'), ...routeFiles].map((p) => readFile(p, 'utf8')),
  )
).join('\n');
