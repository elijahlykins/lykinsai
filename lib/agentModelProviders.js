...
export async function callStructured({
  ...
  // cacheKey is OpenAI-only by design. Anthropic caches through explicit
  // `cache_control` breakpoints (already set on the system block in
  // callAnthropic) and Gemini caches implicitly with nothing to send, so there
  // is no cross-provider caching parameter to unify — only three dialects,
  // which is what this module exists to hold.
  const args = { model, system, user, images, schema, maxTokens, effort, name, signal, cacheKey };
  const startedAt = Date.now();

  let out;
  try {
    if (provider === 'openai' || provider === 'grok') out = await callOpenAi({ ...args, provider });
  ...
  return { ...out, provider: provider || 'unknown', upstreamMs: Date.now() - startedAt };
}
...
export function resolveAgentStageModel({ stage, arm, userId, env = process.env }) {
  ...
  if (stage === 'plan') {
    ...
    // One binary judgement before a turn starts, on a few hundred tokens. It
    // runs in front of the user, so speed matters more than depth.
