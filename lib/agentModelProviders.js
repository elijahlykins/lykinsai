...
async function callOpenAi({ model, system, user, images, schema, maxTokens, effort, name, signal, cacheKey = '', provider = 'openai' }) {
  ...
  // GPT-5/6 families: max_completion_tokens + reasoning_effort, and temperature
  // is rejected. Reasoning tokens count against the cap, so leave headroom.
  const isGpt5Plus = /(?:^|\/)gpt-[56]/.test(model);
  const tokenParams = isGpt5Plus
    ? {
 ...
 */
/**
 * Stages that are the agent THINKING, and therefore follow a bot's pinned
 * model when it has one. The rest — the tool dispatcher, the post-run
 * summariser, the monitors — are plumbing on a few hundred tokens, and
 * dragging them onto a frontier model would spend the user's money without
 * changing a single answer they read.
 */
export const BOT_MODEL_STAGES = Object.freeze(['decide', 'deliver', 'plan', 'verify']);

export function resolveAgentStageModel({ stage, arm, userId, botModelId = '', env = process.env }) {
  if (arm) {
  ...
  // A bot pinned to a specific model uses it for its own reasoning. The
  // caller has already validated the id against the catalog and the user's
  // plan — this function never trusts a raw client value.
  const pinned = String(botModelId || '').trim();
  const base =
    pinned && BOT_MODEL_STAGES.includes(stage)
      ? pinned
      : String(env.BROWSER_AGENT_MODEL || 'gpt-5.6-terra').trim();
  let model = base;
  if (pinned && BOT_MODEL_STAGES.includes(stage)) {
    // Explicit user choice wins over our per-stage tuning. The env hooks
    // exist to tune the DEFAULT; they are not a veto on what the user picked.
  ...
  } else if (stage === 'decide' || stage === 'deliver') {
    // The Bot harness's two hot stages, split so they can be tuned apart:
    // `decide` picks the next step (small output, latency-sensitive, runs
    // every round), `deliver` writes the final message (long output, runs
    // once). Both fall back to the shared base model, so unset behaves
    // exactly as before.
    model = String(
      (stage === 'decide' ? env.BOT_DECIDE_MODEL : env.BOT_DELIVER_MODEL) || base,
    ).trim();
  } else if (stage === 'learn') {
  ...
  // Reasoning effort is per stage too: picking a tool off an index rarely
  // needs the same deliberation as judging whether a run met its goal.
  const effortEnv =
    (stage === 'decide' && env.BOT_DECIDE_REASONING) ||
    (stage === 'deliver' && env.BOT_DELIVER_REASONING) ||
    env.BROWSER_AGENT_REASONING ||
    'low';
  return { model, effort: String(effortEnv).trim() };
}
