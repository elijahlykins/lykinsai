...
describe("classifyAgentSkill", () => {
  ...
  it("creates a new file in-tool; edit-current stays on the open file", () => {
    const {
    ...
    const sheets = getToolVenueById("google-sheets");
    const docs = getToolVenueById("google-docs");
    const sheetHome = "https://docs.google.com/spreadsheets/u/0/";
    const sheetEdit = "https://docs.google.com/spreadsheets/d/abc123/edit";

    assert.equal(venueIsListingOrHome(sheets, sheetHome), true);
    assert.equal(venueHasEditableSurface(sheets, sheetHome), false);
    assert.equal(venueHasEditableSurface(sheets, sheetEdit), true);
    assert.equal(
      venueHasEditableSurface(docs, "https://docs.google.com/document/d/xyz/edit"),
      true,
    );

    // Create → always new file (even if a sheet is already open).
    assert.equal(looksLikeCreateInToolVenueAsk("create a budget", { liveUrl: sheetHome }), true);
    assert.equal(shouldOpenFreshVenueFile("create a budget", sheets, sheetHome), true);
    assert.equal(shouldOpenFreshVenueFile("create a budget", sheets, sheetEdit), true);
    assert.equal(shouldOpenFreshVenueFile("make me an expense tracker", sheets, sheetEdit), true);
    assert.equal(
      classifyAgentSkill("create a budget", { liveUrl: sheetHome, hasLiveTab: true }),
      "tool-create",
    );

    // Edit the open thing → not tool-create, not a fresh file.
    ...
    assert.equal(looksLikeCreateInToolVenueAsk("edit this budget", { liveUrl: sheetEdit }), false);
    assert.equal(looksLikeCreateInToolVenueAsk("add a column for notes", { liveUrl: sheetEdit }), false);
    assert.equal(shouldOpenFreshVenueFile("edit this budget", sheets, sheetEdit), false);
    assert.equal(shouldOpenFreshVenueFile("add a column for notes", sheets, sheetEdit), false);
  });
  ...
  it("offers custom artifact vs stop for complex software like Canva", () => {
    ...
    // Classifies as artifact "build", but the runtime gate must still offer first.
    assert.equal(
  ...
  it("routes named-tool creates to tool-create; plain asks to artifact build", () => {
    ...
    // No external tool named → LYKN artifact / build.
    assert.equal(looksLikeCreateInToolVenueAsk("create for me a budget"), false);
    assert.equal(looksLikeCreateInToolVenueAsk("create me a presentation"), false);
...
describe("agent bookmarks store", () => {
  const fs = require("fs");
  const os = require("os");
  const path = require("path");
  const bookmarks = require("../electron/agentBookmarks.cjs");

  it("toggles star save/remove and builds AI alias map", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "lykn-bm-"));
    try {
      const saved = bookmarks.toggleBookmark(dir, {
      ...
      assert.equal(bookmarks.isBookmarked(saved, saved.items[0].url), true);

      const map = bookmarks.buildAliasMap(saved);
      assert.equal(map["budget sheet"], "https://docs.google.com/spreadsheets/d/xyz/edit");
      assert.ok(map["my budget sheet"] || map["q1 budget"]);

      const removed = bookmarks.toggleBookmark(dir, {
        url: "https://docs.google.com/spreadsheets/d/xyz/edit",
      });
