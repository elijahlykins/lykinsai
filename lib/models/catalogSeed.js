...
function toOpenRouterSlug(provider, upstreamId) {
  ...
  if (provider === 'anthropic') {
    ...
    // Fable point releases (claude-fable-5-1 → claude-fable-5.1) too.
    id = id.replace(/^(claude-(?:fable|opus|sonnet|haiku)-\d+)-(\d+)$/, '$1.$2');
  }
  return id;
}
...
function contextWindowFor(id) {
  const m = String(id);
  if (/^gpt-6-astra/.test(m)) return 1_050_000;
  ...
  if (/^claude-fable-5/.test(m)) return 1_000_000;
