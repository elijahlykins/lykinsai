...
test('a sandbox notification verifies', async () => {
...
test('a sandbox payload is never handed to the production verifier', async () => {
  // Regression. The verifier used to be chosen by trying production first and
  // falling back on INVALID_ENVIRONMENT — which never fired, because
  // SignedDataVerifier checks the app identifier FIRST and real sandbox
  // notifications omit appAppleId, so they failed as INVALID_APP_IDENTIFIER.
  // Every sandbox notification was rejected, including App Review's.
  //
  // A production verifier that would reject anything proves the routing:
  // if the sandbox payload still verifies, it never went near it.
  const exploding = new SignedDataVerifier(
    [testRoot], false, Environment.PRODUCTION, 'com.example.never-matches', 1,
  );
  const result = await verifyAppleNotification(fixtures.sandbox, {
    production: exploding,
    sandbox: verifiers().sandbox,
  });
  assert.equal(result.environment, 'Sandbox');
});

test('a production payload is never handed to the sandbox verifier', async () => {
  // The mirror image. The sandbox verifier does not check appAppleId, so
  // routing production traffic to it would silently drop that check.
  const exploding = new SignedDataVerifier(
    [testRoot], false, Environment.SANDBOX, 'com.example.never-matches',
  );
  const result = await verifyAppleNotification(fixtures.production, {
    production: verifiers().production,
    sandbox: exploding,
  });
  assert.equal(result.environment, 'Production');
});

test('a payload cannot lie about its environment to dodge the appAppleId check', async () => {
  // Selecting the verifier from unverified data is only safe because the
  // choice cannot bypass anything: data.environment is inside the signature,
  // and the chosen verifier re-checks it. Claiming Sandbox on a
  // production-signed payload therefore fails rather than skipping the check.
  const [head, body, sig] = fixtures.production.split('.');
  const decoded = JSON.parse(Buffer.from(body, 'base64url').toString());
  decoded.data.environment = 'Sandbox';
  const relabelled = Buffer.from(JSON.stringify(decoded)).toString('base64url');
  await assert.rejects(
    () => verifyAppleNotification(`${head}.${relabelled}.${sig}`, verifiers()),
  );
});

test('a real Apple sandbox notification verifies against the real Apple root', async () => {
  // Captured from Apple on 2026-08-07. Not a fixture we minted — a genuine
  // signed payload, verified here against certs/apple, exactly as production
  // does it. This is the case every synthetic fixture got wrong.
  const real = JSON.parse(
    readFileSync(path.join(HERE, '__fixtures__/apple-sandbox-test-notification.json'), 'utf8'),
  );
  const appleRoots = loadAppleRootCertificates();
  const result = await verifyAppleNotification(real.signedPayload, {
    production: new SignedDataVerifier(appleRoots, false, Environment.PRODUCTION, 'io.lykn.app', 6765728365),
    sandbox: new SignedDataVerifier(appleRoots, false, Environment.SANDBOX, 'io.lykn.app'),
  });

  assert.equal(result.environment, 'Sandbox');
  assert.equal(result.payload.notificationType, 'TEST');
  assert.equal(result.payload.data.bundleId, 'io.lykn.app');
  // The omission that caused the bug. If Apple ever starts sending it, this
  // fails and the fixture should be recaptured rather than the assert relaxed.
  assert.equal(result.payload.data.appAppleId, undefined);
  // A TEST notification carries no transaction and must not touch billing.
  assert.equal(result.transaction, null);
  assert.equal(appleSyncInputFrom(result), null);
});
