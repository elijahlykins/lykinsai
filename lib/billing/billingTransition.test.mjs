...
test('new Pro checkout uses $20 configuration and Max stays $100', () => {
  ...
  const proPlus = PLANS.find((plan) => plan.id === 'pro_plus');
  ...
  assert.equal(proPlus.monthlyPrice, 60);
  assert.equal(proPlus.annualPrice, 612);
  assert.equal(proPlus.checkout, true);
  assert.equal(proPlus.listed, false);
  assert.equal(listedPlans().some((plan) => plan.id === 'pro_plus'), false);
  assert.equal(plansForPicker('studio').some((plan) => plan.id === 'pro_plus'), false);
  assert.equal(plansForPicker('pro_plus').some((plan) => plan.id === 'pro_plus'), true);
