 ...
 *
 * Existing subscribers whose current period was paid before usage-v3 never
 * received that grant. `recoverCurrentPeriodPlanGrant` replays the same
 * invoice-id grant (or an Apple/admin period grant) so the next request
 * funds them without waiting for the next renewal.
 */
...
/** End of the current UTC month. Used as the period for comped / admin grants. */
export function calendarMonthPeriodEndIso(now = new Date()) {
...
function fallbackPeriodEndUnix(fallbackPeriodEnd) {
  if (!fallbackPeriodEnd) return null;
  if (typeof fallbackPeriodEnd === 'number' && Number.isFinite(fallbackPeriodEnd) && fallbackPeriodEnd > 0) {
    return fallbackPeriodEnd > 1e12 ? Math.floor(fallbackPeriodEnd / 1000) : Math.floor(fallbackPeriodEnd);
  }
  const ms = Date.parse(fallbackPeriodEnd);
 ...
 */
export function classifyPlanFundingInvoice({ invoice, planId, fallbackPeriodEnd } = {}) {
  ...
  const periodEndUnix = invoicePeriodEnd(invoice) || fallbackPeriodEndUnix(fallbackPeriodEnd);
 ...
 */
export async function grantPlanUsageFromInvoice({ userId, invoice, planId, fallbackPeriodEnd } = {}) {
  if (!userId) return { ok: false, error: 'missing_user' };

  const decision = classifyPlanFundingInvoice({ invoice, planId, fallbackPeriodEnd });
  if (!decision.fund) {
...
/**
 * First qualifying paid invoice in a Stripe list. Pure.
 */
export function pickCurrentPlanFundingInvoice(invoices, planId, fallbackPeriodEnd) {
  for (const invoice of invoices || []) {
    const decision = classifyPlanFundingInvoice({ invoice, planId, fallbackPeriodEnd });
    if (decision.fund) return { invoice, decision };
  }
  return null;
}

/**
 * Catalog-sized grant for a paid period with no Stripe invoice (Apple IAP
 * or an admin-granted paid row). Idempotent per user + period-end day.
 * Never use this for a Stripe customer — a later invoice.paid would double.
 */
export function classifyPeriodPlanGrant({ planId, periodEnd, billingPeriod } = {}) {
  const plan = resolvePlanId(planId);
  if (!isPaidPlan(plan)) return { fund: false, reason: 'not_a_paid_plan' };

  const periodEndUnix = fallbackPeriodEndUnix(periodEnd);
  if (!periodEndUnix || periodEndUnix * 1000 <= Date.now()) {
    return { fund: false, reason: 'period_already_ended' };
  }

  const catalog = getPlan(plan);
  const annual = String(billingPeriod || '').toLowerCase() === 'annual';
  const cents = annual ? catalog.annualCents : catalog.monthlyCents;
  if (!Number.isInteger(cents) || cents <= 0) {
    return { fund: false, reason: 'zero_amount' };
  }
  if (cents > MAX_PLAN_GRANT_CENTS) {
    return { fund: false, reason: 'amount_exceeds_ceiling', cents };
  }
...
export async function grantPlanUsageForPeriod({
  userId,
  planId,
  periodEnd,
  billingPeriod,
  ...
  const decision = classifyPeriodPlanGrant({ planId, periodEnd, billingPeriod });
  if (!decision.fund) {
    logBillingEvent('plan_funding_skipped', {
      userId,
      reason: decision.reason,
  ...
  const result = await grantUsageBalance(userId, {
    ...
    txnType: TXN_TYPES.SUBSCRIPTION_GRANT,
    expiresAt: decision.expiresAt,
    idempotencyKey: periodPlanGrantIdempotencyKey(userId, decision.expiresAt),
    metadata: {
      source: `period-recover:${source}`,
      plan: decision.planId,
      plan_label: getPlan(decision.planId).label,
      period_end_unix: decision.periodEndUnix,
    },
...
/**
 * Recover the current period's plan usage when the original invoice.paid
 * (or Apple renewal) landed before the usage ledger existed.
 *
 * Stripe customers are funded only from a qualifying paid invoice, using
 * the same invoice-id idempotency as the webhook. Apple / admin rows use
 * a period-end key. Already-granted current plan lots are a no-op.
 */
export async function recoverCurrentPeriodPlanGrant({
  userId,
  planId,
  periodEnd,
  billingPeriod,
  ...
  stripeCustomerId,
  invoices = [],
  hasCurrentPlanGrant = false,
} = {}) {
  if (!userId) return { ok: false, error: 'missing_user' };
  if (hasCurrentPlanGrant) return { ok: true, skipped: true, reason: 'already_granted' };
  if (!isPaidPlan(planId)) return { ok: true, skipped: true, reason: 'not_a_paid_plan' };

  const picked = pickCurrentPlanFundingInvoice(invoices, planId, periodEnd);
  if (picked) {
    return grantPlanUsageFromInvoice({
      userId,
  ...
  const isStripe = provider === 'stripe' || Boolean(stripeCustomerId);
  if (isStripe) {
    logBillingEvent('plan_funding_skipped', {
      userId,
      reason: 'no_qualifying_invoice',
    });
    return { ok: true, skipped: true, reason: 'no_qualifying_invoice' };
  }

  return grantPlanUsageForPeriod({
    userId,
    planId,
    periodEnd,
    billingPeriod,
    source: provider === 'apple' ? 'apple' : 'manual',
  });
}
