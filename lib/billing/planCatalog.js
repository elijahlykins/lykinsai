...
/** Fallback key when there is no Stripe invoice (Apple IAP, admin-granted). */
export function periodPlanGrantIdempotencyKey(userId, periodEnd) {
  const stamp = String(periodEnd || '').slice(0, 10) || 'none';
  return `plan-grant:period:${userId}:${stamp}`;
}
