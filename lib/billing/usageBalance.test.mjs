...
import { classifyPlanFundingInvoice, grantPlanUsageFromInvoice, recoverCurrentPeriodPlanGrant, calendarMonthPeriodEndIso } from './planFunding.js';
...
test('existing Pro with no plan grant recovers usage from the paid invoice', withStore(async () => {
  const result = await recoverCurrentPeriodPlanGrant({
    userId: userA,
    planId: 'studio',
    periodEnd: new Date(Date.now() + 20 * 24 * 3600 * 1000).toISOString(),
    provider: 'stripe',
  ...
  const again = await recoverCurrentPeriodPlanGrant({
    userId: userA,
    planId: 'studio',
    provider: 'stripe',
    stripeCustomerId: 'cus_test',
...
test('Stripe recovery does not invent a grant when no invoice qualifies', withStore(async () => {
  const result = await recoverCurrentPeriodPlanGrant({
    userId: userA,
    planId: 'studio',
    periodEnd: new Date(Date.now() + 20 * 24 * 3600 * 1000).toISOString(),
    provider: 'stripe',
    stripeCustomerId: 'cus_test',
    invoices: [paidInvoice({ amount_paid: 0, total_excluding_tax: 0 })],
    hasCurrentPlanGrant: false,
  });
  assert.equal(result.skipped, true);
  assert.equal(result.reason, 'no_qualifying_invoice');
  assert.equal((await getUsageBalance(userA)).plan, 0);
}));

test('already-granted current period is a no-op', withStore(async () => {
  const result = await recoverCurrentPeriodPlanGrant({
    userId: userA,
    planId: 'studio',
    invoices: [paidInvoice()],
    hasCurrentPlanGrant: true,
  });
  assert.equal(result.skipped, true);
...
test('Apple / admin paid rows recover catalog usage for the current period', withStore(async () => {
  ...
  const dup = await recoverCurrentPeriodPlanGrant({
    userId: userA,
    planId: 'studio',
    periodEnd,
    billingPeriod: 'monthly',
...
test('calendar month period end is the first instant of next UTC month', () => {
  assert.equal(
    calendarMonthPeriodEndIso(new Date('2026-09-09T04:00:00Z')),
    '2026-10-01T00:00:00.000Z',
  );
});

test('comped Pro with no Stripe invoice recovers the catalog $20 for the calendar month', withStore(async () => {
  const result = await recoverCurrentPeriodPlanGrant({
    userId: userA,
    planId: 'studio',
    periodEnd: calendarMonthPeriodEndIso(),
    billingPeriod: 'monthly',
    provider: null,
...
test('invoice without line periods still funds when billing period end is in the future', () => {
  const fallback = new Date(Date.now() + 10 * 24 * 3600 * 1000).toISOString();
  const decision = classifyPlanFundingInvoice({
    invoice: paidInvoice({ lines: { data: [] }, period_end: 0 }),
    planId: 'studio',
    fallbackPeriodEnd: fallback,
  });
  assert.equal(decision.fund, true);
  assert.equal(decision.cents, 2000);
});
