 ...
 *   ensureSignupGrant            — one-time $20 promotional usage
...
import { grantUnlimitedUsage } from './internalAccounts.js';
...
// Every account receives $20 of promotional usage exactly once. Idempotent
...
export async function authorizeImageUsage(userId, planId = 'free', actionType = 'image_gen', email = null) {
...
export async function authorizeMeteredUsage({
  ...
  const unlimited = unlimitedUsage || grantUnlimitedUsage({ userId, email });
...
export async function recordUsageAfterLog({
  ...
  const unlimited = unlimitedUsage || grantUnlimitedUsage({ userId, email });
  if (isIncludedSubscriptionUsage({ unlimitedUsage: unlimited })) {
    return {
      ok: true,
      skipped: true,
...
export function customerUsagePayload(balance) {
