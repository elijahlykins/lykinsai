...
/** Stamp project focus to a user-owned project (custom model bind). */
...
export function formatBoundProjectGuidance(project) {
  ...
  return [
    ...
    'ALL lykn_pushProjectState calls MUST include',
 ...
 *
 * In-app LYKN (Mac chat, Glass, voice) always brands as "LYKN" — never the
 * underlying frontier/custom model name. Explicit automation labels
 * (night-shift, cursor-build) and sub-agent custom-model names are preserved.
 */
