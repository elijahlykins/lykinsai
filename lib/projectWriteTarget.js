...
/**
 * Attribution stamp for project writes (`set_by_client` / steward `source`).
 *
 * In-app LYKN (Mac chat, Glass, voice JWT) always brands as "LYKN" — never the
 * underlying frontier/custom model name. External MCP tokens keep their
 * clientKind. Explicit automation labels (night-shift, cursor-build) and
 * sub-agent custom-model names are preserved.
 */
export function resolveProjectPushClient(ctx) {
  const mcpKind = ctx?.mcpAuth?.clientKind
    ? String(ctx.mcpAuth.clientKind).trim().slice(0, 80)
    : '';
  if (mcpKind) return mcpKind;

  const label = ctx?.chatModelLabel
    ? String(ctx.chatModelLabel).trim().slice(0, 80)
    : '';
  if (label === 'cursor-build' || label === 'night-shift' || label === 'user') {
    return label;
  }
  if (ctx?.attribSurface === 'sub-agent' && label) return label;

  // JWT / in-app surfaces (text chat, Glass, voice) — brand as LYKN.
  if (ctx?.attribSurface === 'lykn-chat' || !ctx?.mcpAuth) return 'LYKN';
