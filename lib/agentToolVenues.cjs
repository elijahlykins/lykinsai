 ...
 */
function looksLikeEditCurrentInToolAsk(text) {
  ...
  const pointsAtOpen =
    /\b(this|that|these|those|the\s+open|the\s+current|here|in\s+here|on\s+this)\b/i.test(
      lower,
    ) ||
    /\bthe\s+(sheet|spreadsheet|doc|document|deck|presentation|slides?|table|grid|file|design|page)\b/i.test(
      lower,
    );
  ...
  if (
    ...
    /\b(column|columns|row|rows|cell|cells|tab|sheet|section|heading|title|slide|slides|page|field|formula|chart|header)\b/i.test(
      lower,
    )
  ) {
    return true;
  ...
  if (
    /\b(edit|change|update|tweak|adjust|modify|revise|improve|fix|restyle|rewrite|replace|paste|put|enter|fill)\b/i.test(
      lower,
    ) &&
    pointsAtOpen
  ) {
  ...
  if (/\bmake\s+(this|that|it|the\s+\w+)\b/i.test(lower) && pointsAtOpen) {
    return true;
  }
...
function shouldOpenFreshVenueFile(text, venue, url) {
  ...
  if (looksLikeEditCurrentInToolAsk(text)) return false;
  const u = String(url || "");
...
function matchCreateInToolVenue(text, opts = {}) {
  ...
  if (looksLikeEditCurrentInToolAsk(t)) return null;
...
function buildToolActAdaptiveGoal(venue, ask, { draft = "" } = {}) {
  ...
  const draftBit = cleanDraft
    ? `\n\nDRAFTED CONTENT TO PLACE IN THE TOOL (use this — type/paste into fields):\n${cleanDraft.slice(0, 3500)}\n`
    : "";
