...
import { applySignupGrant } from '../billing/applySignupGrant.js';
...
export function buildSignupCodeEmailHtml({ code, name }) {
  ...
  return wrapAuthEmailHtml({ title: 'Confirm your email', bodyHtml });
}
...
export function createEmailSignupHandlers(deps) {
  ...
  async function verifySignupCode({ email: rawEmail, code: rawCode }) {
    ...
    try {
      await applySignupGrant({ userId: user.id, supabaseAdmin });
    } catch (err) {
      console.warn('⚠️ signup grant on verify failed:', err?.message || err);
    }
