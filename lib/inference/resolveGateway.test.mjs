...
test('OPENROUTER_API_KEY routes curated models through OpenRouter by default', () => {
  ...
  const astra = resolveInferenceTarget('gpt-6-astra', env);
  assert.equal(astra.gateway, 'openrouter');
  assert.equal(astra.upstreamId, 'openai/gpt-6-astra');

  const fable = resolveInferenceTarget('claude-fable-5.1', env);
  assert.equal(fable.gateway, 'openrouter');
  assert.equal(fable.upstreamId, 'anthropic/claude-fable-5.1');
...
test('catalog mapper strips OpenRouter free marks from labels', () => {
  const mapped = mapOpenRouterModel({
  ...
  assert.equal(mapped.label, 'Llama 3.3 70B Instruct');
  assert.equal(mapped.pricing.input, 0);
  assert.equal(mapped.pricing.output, 0);
});
