...
import { safeFetch } from '../exterior/ssrfGuard.js';
...
function classifyConnectError(error) {
  if (error instanceof UnauthorizedError || error?.status === 401 || /401|unauthorized/i.test(String(error?.message || ''))) {
    return { code: 'authentication_required', message: 'Authentication required (Phase 2 OAuth is not implemented).' };
  }
...
export function createGuardedFetch({ trustLevel, signal } = {}) {
  return async (url, init = {}) => {
    const safe = await assertMcpUrlSafe(String(url), { trustLevel });
    if (!safe.ok) {
    ...
    if (trustLevel === MCP_TRUST_LEVELS.LOCAL_TRUSTED) {
      return fetch(safe.url, merged);
    }
    return safeFetch(safe.url, merged);
  };
}

export async function createMcpClientRuntime({
  serverUrl,
  trustLevel = MCP_TRUST_LEVELS.REMOTE,
  headers = {},
  signal,
  fetchImpl,
} = {}) {
...
export { classifyConnectError, UnauthorizedError };
