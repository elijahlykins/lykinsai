/**
 * Standards-compliant MCP client runtime.
 *
 * Streamable HTTP for remote servers. Official SDK stdio for local servers.
 * Tool discovery is dynamic. Resources and prompts are optional.
 */

import { Client } from '@modelcontextprotocol/sdk/client/index.js';
import { StreamableHTTPClientTransport } from '@modelcontextprotocol/sdk/client/streamableHttp.js';
import { UnauthorizedError } from '@modelcontextprotocol/sdk/client/auth.js';
import { MCP_CLIENT_NAME, MCP_CLIENT_VERSION, MCP_TRANSPORTS, MCP_TRUST_LEVELS } from './protocol.js';
...
function pageLimit(items, max = BOUNDS.MAX_TOOLS_DISCOVERED) {
...
async function createTransport({
  ...
  if (transportKind === MCP_TRANSPORTS.STDIO || stdioTransport || stdio) {
    if (stdioTransport) return stdioTransport;
    const { StdioClientTransport } = await import('@modelcontextprotocol/sdk/client/stdio.js');
    return new StdioClientTransport({
      command: stdio.command,
      args: Array.isArray(stdio.args) ? stdio.args : [],
      env: stdio.env,
      cwd: stdio.cwd || undefined,
      stderr: 'pipe',
    });
  }
  ...
  return new StreamableHTTPClientTransport(new URL(urlCheck.url), {
    requestInit: {
      headers: { ...headers },
    },
...
export async function createMcpClientRuntime({
  ...
  fetchImpl,
  authProvider,
  transportKind = MCP_TRANSPORTS.STREAMABLE_HTTP,
  stdioTransport,
  stdio,
} = {}) {
  ...
  async function listAll(method, key) {
    ...
    return selectToolsForCache(pageLimit(out), BOUNDS.MAX_TOOLS_CACHED);
  }
