...
export function createCredentialRef(input = {}) {
  ...
  if (type === CREDENTIAL_REF_TYPES.MCP_SECRET) {
    const connectionId = String(input.connectionId || '').trim();
    if (!connectionId) throw new TypeError('mcp_secret credentialRef requires connectionId');
    return Object.freeze({ type: CREDENTIAL_REF_TYPES.MCP_SECRET, connectionId });
  }
...
export function publicCredentialRef(ref) {
  ...
  if (ref.type === CREDENTIAL_REF_TYPES.MCP_SECRET) {
    return { type: CREDENTIAL_REF_TYPES.MCP_SECRET, connectionId: String(ref.connectionId) };
  }
