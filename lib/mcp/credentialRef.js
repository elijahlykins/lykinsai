...
export function redactValue(value) {
  ...
  if (!text) return text;
  // A single opaque blob (no whitespace) that mentions token/secret material
  // is redacted whole. Prose or serialized JSON that merely CONTAINS the word
  // "token" (e.g. a Gmail response with nextPageToken) must survive - only
  // the secret-shaped substrings inside it are scrubbed.
  if (!/\s/.test(text) && text.length > 24 && /secret|token|bearer/i.test(text)) {
    return '[redacted]';
  }
  return text.replace(new RegExp(SECRET_VALUE_RE.source, 'gi'), '[redacted]');
}
