 ...
 *   Local stdio uses the official SDK StdioClientTransport. The client
 *   runtime is transport-pluggable; discovery, trust, and credentials share
 *   one manager.
 *
 * Auth:
...
/**
 * Connections whose endpoint URL and auth headers are minted live from the
 * LYKN Connection Service (Composio-managed OAuth apps). The stored
 * serverUrl is a stable placeholder for identity/display; the session URL
 * and headers are capability credentials and are never persisted.
 */
export const MANAGED_TOOL_PROVIDER = 'composio';

export function isManagedToolConnection(row) {
  return row?.providedThrough === MANAGED_TOOL_PROVIDER;
}

export const MCP_DEFAULT_TRANSPORT = MCP_TRANSPORTS.STREAMABLE_HTTP;
export const MCP_PROTOCOL_VERSION = '2025-06-18';

export const CLASSIFIER_VERSION = 'deterministic_v5';
