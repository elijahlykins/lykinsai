...
import { MCP_AUTH_MODES, MCP_STATUSES, MCP_TRUST_LEVELS, MCP_TRANSPORTS, AUTH_REQUIRED_STATUSES } from './protocol.js';
...
export function createMcpConnectionManager({
  ...
  async function connect(userId, input = {}) {
    ...
    try {
      const auth = stdio ? { headers: {} } : await resolveAuth(row);
      const runtime = stdio
        ? await startStdioRuntime({ ...row, ...stdioSpec }, { confirmInstall: true })
        : await createMcpClientRuntime({
            serverUrl: row.serverUrl,
            trustLevel: row.trustLevel,
