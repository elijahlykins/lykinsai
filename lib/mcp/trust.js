...
export function neutralizeUntrustedInstructionText(raw) {
  let text = String(raw || '');
...
export function sanitizeToolDescription(raw) {
  ...
  let text = neutralizeUntrustedInstructionText(clipped.text.replace(/\s+/g, ' ').trim());
  return {
    text,
...
export function wrapUntrustedObservation(payload, { connectionId, toolName } = {}) {
  // Redact BEFORE bounding. Once boundJson truncates, the payload is one
  // giant preview string and key-aware redaction can no longer distinguish
  // a credential field from message content.
  const bounded = boundJson(redactDeep(payload), MCP_BOUNDS.TOOL_RESULT_BYTES);
  // Shrinking may have parsed an embedded JSON string into structure that the
  // first key-aware pass could not see into — redact the final shape too.
  const data = bounded.shrunk ? redactDeep(bounded.value) : bounded.value;
