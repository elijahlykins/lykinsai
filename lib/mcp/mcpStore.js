 ...
 * Public rows never include secret_encrypted or raw tokens.
 */
  ...
  return {
    async list(userId) {
      return [...rows.values()]
        ...
        .map((row) => ({ ...row, classifiedTools: row.classifiedTools || [] }));
    },
    async get(userId, id) {
    ...
    async insert(userId, input) {
      ...
      const row = {
        ...
        credentialRef: createCredentialRef(
          input.authMode === MCP_AUTH_MODES.BEARER
            ? { type: CREDENTIAL_REF_TYPES.MCP_SECRET, connectionId: id }
            : input.credentialRef || { type: CREDENTIAL_REF_TYPES.NONE },
        ),
        secretEncrypted: input.secretEncrypted || null,
        trustLevel: input.trustLevel || MCP_TRUST_LEVELS.REMOTE,
...
export function createSupabaseMcpStore(supabaseAdmin, { encrypt, decrypt } = {}) {
  ...
  function fromRow(row) {
    if (!row) return null;
    const credentialRef =
      row.auth_mode === MCP_AUTH_MODES.BEARER
        ? createCredentialRef({ type: CREDENTIAL_REF_TYPES.MCP_SECRET, connectionId: row.id })
        : createCredentialRef({ type: CREDENTIAL_REF_TYPES.NONE });
    return {
      id: row.id,
      userId: row.user_id,
      name: row.name,
      ...
      classifiedTools: row.classified_tools || [],
  ...
  return {
    ...
    async list(userId) {
      const { data, error } = await supabaseAdmin
        ...
        .select(
          'id, user_id, name, server_url, transport, auth_mode, trust_level, server_info, capability_summary, classified_tools, schema_hash, status, last_error, created_at, updated_at, last_connected_at',
        )
        .eq('user_id', userId)
