  ...
  return {
    ...
    async list(userId) {
      const { data, error } = await supabaseAdmin
        ...
        .select(
          'id, user_id, name, server_url, transport, auth_mode, trust_level, server_info, capability_summary, classified_tools, schema_hash, status, last_error, created_at, updated_at, last_connected_at, account_label, account_identity, origin, identity, session_epoch',
        )
        .eq('user_id', userId)
