...
/**
 * HTTP callers cannot attest standing authorization or expand a Routine
 * allowlist. Desktop TaskCompiler remains the only path that copies a
 * persisted Routine approval policy onto a Task.
 */
...
function classifiedToolName(tool) {
  return String(tool?.toolName || tool?.serverToolName || tool?.name || '').trim();
}

/**
 * An explicit HTTP / bot `tools/call` names the tool. Chat's search→call
 * path already pins that one tool into the resolution; the disclosure
 * ranking (top 10) must not reject a catalog tool the caller asked for.
 */
export function resolutionWithExplicitTool(resolution, { connection, toolName } = {}) {
  const want = String(toolName || '').trim();
  const ranked = resolution && typeof resolution === 'object' ? resolution : { tools: [] };
  if (!connection?.id || !want) return ranked;
  const classified = (connection.classifiedTools || []).find((t) => classifiedToolName(t) === want)
    || (connection.classifiedTools || []).find(
      (t) => classifiedToolName(t).toLowerCase() === want.toLowerCase(),
    );
  if (!classified) return ranked;
  const pinnedName = classifiedToolName(classified);
  const pinned = {
    ...
    toolName: pinnedName,
    semanticCapabilities: classified.semanticCapabilities || classified.capabilities,
    consequenceHint: classified.consequenceHint || classified.consequence,
    consequence: classified.consequence || classified.consequenceHint,
    confidence: classified.confidence,
    description: classified.description,
    inputSchema: classified.inputSchema,
    schemaFingerprint: classified.schemaFingerprint,
    classifierVersion: classified.classifierVersion,
  };
  const rest = (ranked.tools || []).filter(
    (item) => !(item.connectionId === connection.id && item.toolName === pinnedName),
  );
  return { ...ranked, ok: true, tools: [pinned, ...rest] };
}
