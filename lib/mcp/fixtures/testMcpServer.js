...
function createFixtureMcp(extraTools = [], { slowWriteMs = 0, includeDefaults = true } = {}) {
...
export { createFixtureMcp, delay };

export async function startFixtureMcpServer({ extraTools = [], slowWriteMs = 0, requireAuth = false, includeDefaults = true } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (!session) {
      const mcp = createFixtureMcp(extraTools, { slowWriteMs, includeDefaults });
