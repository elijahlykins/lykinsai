...
test('without an arm, production stage routing is unchanged', () => {
  ...
  assert.equal(resolveAgentStageModel({ stage: 'monitor_semantic', arm: '', env }).model, 'gpt-4.1-mini');
  assert.equal(
    resolveAgentStageModel({
      stage: 'monitor_vision',
      arm: '',
      env: { ...env, MONITOR_VISION_MODEL: 'gpt-4.1-mini' },
    }).model,
    'gpt-4.1-mini',
  );
  assert.equal(resolveAgentStageModel({ stage: 'describe', arm: '', env }).model, 'gpt-5.6-terra');
  assert.equal(
    resolveAgentStageModel({
      stage: 'describe',
      arm: '',
      env: { ...env, DESCRIBE_MEDIA_MODEL: 'gpt-4.1-mini' },
    }).model,
    'gpt-4.1-mini',
  );
});
...
test('OPENROUTER_API_KEY sends structured calls through OpenRouter', async () => {
  const { callStructured } = await import('./agentModelProviders.js');
  const seen = [];
  const realFetch = globalThis.fetch;
  const prevOr = process.env.OPENROUTER_API_KEY;
  ...
  try {
    await callStructured({ model: 'grok-4.6', user: 'u', schema: { type: 'object' }, name: 'n' });
    assert.match(seen[0].url, /openrouter\.ai/);
    assert.equal(seen[0].auth, 'Bearer sk-or');
...
test('parseJsonLoose survives two concatenated JSON objects', async () => {
  // The exact live failure behind "agent model unavailable (502)":
  // gpt-5.6-terra through OpenRouter (strict:false json_schema) emitted an
  // `act` decision AND a speculative `finish` in one completion, roughly one
  // decide call in five. The old first-{-to-last-} slice spanned both objects
  // and returned null, so a schema-valid decision failed the whole round.
  const { parseJsonLoose } = await import('./agentModelProviders.js');
  const doubled =
    '{"kind":"act","action":{"type":"navigate","url":"https://www.perplexity.ai/"},"reason":"Open it.","risk":"low"}\n\n' +
    '{"kind":"finish","answer":"Opened the Perplexity landing page."}';
  const out = parseJsonLoose(doubled);
  assert.equal(out.kind, 'act');
  assert.equal(out.action.url, 'https://www.perplexity.ai/');
});

test('parseJsonLoose still handles clean JSON, prose wrapping, and fences', async () => {
  const { parseJsonLoose } = await import('./agentModelProviders.js');
  ...
  assert.deepEqual(parseJsonLoose('{"note":"set {x} to \\"}\\" first"}{"second":true}'), {
    note: 'set {x} to "}" first',
  });
