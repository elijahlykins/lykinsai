...
// stores an encrypted, typed generic credential. We persist its credential_id
// on each build so completion polling uses the same key that launched the run.
// Legacy social_connections rows are read only to migrate existing users.
//
...
/**
 * Resolve the Cursor API key for a user's build. Generic credentials are
 * canonical. A legacy social_connections row is copied on first use so
 * existing users are not stranded during rollout.
 *
 * @param {object} client   Supabase admin client
 * @param {string} userId
 * @param {{ credentialId?: string|null, legacyConnectionId?: string|null }} opts
 * @returns {Promise<{ apiKey: string, credentialId: string|null, legacyConnectionId: string|null, defaultRepo: string|null } | null>}
 */
async function resolveCursorCredential(
  client,
  userId,
  { credentialId = null, legacyConnectionId = null } = {},
) {
  if (!client || !userId) return null;

  const store = createSupabaseCredentialStore(client);
  try {
    const generic = credentialId
      ? await store.get(userId, credentialId, { includeSecret: true })
      : await store.findActive(userId, CREDENTIAL_TYPES.CURSOR_CLOUD_API_KEY, {
          includeSecret: true,
        });
    if (generic?.secret) {
      void store.update(userId, generic.id, {
        lastUsedAt: new Date().toISOString(),
      }).catch(() => {});
      return {
        apiKey: String(generic.secret).trim(),
        credentialId: generic.id,
        legacyConnectionId: null,
        defaultRepo: generic.metadata?.default_repo
          ? normalizeRepo(generic.metadata.default_repo)
          : null,
      };
    }
  } catch (error) {
    // A missing migration during a rolling deploy must not strand active
    // builds. The read-only legacy path below remains available temporarily.
    console.warn('[cursorBuilds] generic credential lookup failed:', error?.message || error);
  }
  ...
  if (error || !Array.isArray(data) || data.length === 0) {
    if (legacyConnectionId) return resolveCursorCredential(client, userId, {});
    return null;
  }
  ...
  let migrated = null;
  try {
    migrated = await store.put(userId, {
      type: CREDENTIAL_TYPES.CURSOR_CLOUD_API_KEY,
      secret: apiKey,
      label: row.metadata?.key_name || 'Cursor Cloud',
      metadata: {
        default_repo: row.metadata?.default_repo || null,
        migrated_from_social_connection_id: row.id,
      },
    });
  } catch (migrationError) {
    console.warn('[cursorBuilds] legacy credential migration failed:', migrationError?.message || migrationError);
  }

  return {
    apiKey,
    credentialId: migrated?.id || null,
...
export async function launchCursorBuild({
  ...
  const row = {
    ...
    credential_id: cred.credentialId || null,
    connection_id: cred.credentialId ? null : cred.legacyConnectionId,
...
export async function syncCursorBuild(client, build) {
  ...
  // Poll with the same key that launched the run. connection_id is consulted
  // only for old builds created before generic credentials were introduced.
  const cred = await resolveCursorCredential(client, build.user_id, {
    credentialId: build.credential_id || null,
    legacyConnectionId: build.connection_id || null,
  });
