...
export function assertUserPath(userId, storagePath) {
  const uid = String(userId || '').trim();
  const path = String(storagePath || '').trim();
  if (!uid || !path) return { ok: false, error: 'storage_path_required' };
  // Reject traversal BEFORE the prefix test: `${uid}/../other/file` starts
  // with `${uid}/` and would otherwise pass straight through.
  if (path.includes('..')) return { ok: false, error: 'storage_path_forbidden' };
...
export function capabilityCtx(ctx = {}) {
  return {
    ...
    authorizeUsage: typeof ctx.authorizeUsage === 'function' ? ctx.authorizeUsage : null,
    ...
    // Multi-file project source. Dropping this used to make file_ops fail with
    // "pass files in the same call" even though [ARTIFACT_OPEN] listed them,
    // and the edit schema had already stripped `files` so the model could not.
    activeArtifactFiles: Array.isArray(ctx.activeArtifactFiles)
      ? ctx.activeArtifactFiles
      : ctx.activeArtifactFiles && typeof ctx.activeArtifactFiles === 'object'
        ? ctx.activeArtifactFiles
        : null,
    activeArtifactEntry:
      typeof ctx.activeArtifactEntry === 'string' && ctx.activeArtifactEntry.trim()
        ? ctx.activeArtifactEntry.trim()
        : null,
    activeArtifactTodos: Array.isArray(ctx.activeArtifactTodos) ? ctx.activeArtifactTodos : null,
    activeArtifactRuntimeErrors: Array.isArray(ctx.activeArtifactRuntimeErrors)
      ? ctx.activeArtifactRuntimeErrors
      : null,
    ...
    // true only when the user message matches redesign/rebuild intent —
    // builders reject full_rewrite otherwise.
    allowFullRewrite: ctx.allowFullRewrite === true,
    // true when the user asked to change colors/fonts/theme (or redesign).
    allowStyleChange: ctx.allowStyleChange === true,
