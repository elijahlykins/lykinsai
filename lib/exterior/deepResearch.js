...
export function formatDeepResearchForPrompt(pack) {
  ...
  const embedInstructions =
    ...
    `   CRITICAL — Every \`\`\`stock / \`\`\`chart / \`\`\`sheet fence MUST be closed with \`\`\` before ` +
    `you write more prose. Never leave an open fence. Prefer finishing (or skipping) an embed over ` +
    `starting a new one near the end. Write the **Sources** section last and always complete it.`
