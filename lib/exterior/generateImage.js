...
import { buildFileProxyUrl, fileProxyBaseUrl, verifyFileToken } from './fileProxy.js';
...
async function callOpenAiImage({ apiKey, prompt, aspectRatio, imageSize, referenceImages }) {
  ...
  try {
    ...
    return { ok: false, error: e?.message || 'openai_network_error', model };
  }
  ...
  return { ok: false, error: 'model_returned_no_image', model };
}
...
async function loadFileProxyReference(url, supabaseAdmin) {
  ...
  try {
    if (parsed.origin !== new URL(fileProxyBaseUrl()).origin) return null;
  } catch {
    return null;
  }
...
export async function generateChatImage({
  ...
  if (Array.isArray(referenceImages) && referenceImages.length > 0) {
    console.log(`🖼 image-gen: ${refs.length}/${referenceImages.length} reference image(s) usable`);
  }
