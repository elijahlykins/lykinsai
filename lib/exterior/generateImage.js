...
async function callOpenAiImage({ apiKey, prompt, aspectRatio, imageSize, referenceImages, model: requestedModel }) {
  const model = String(requestedModel || OPENAI_IMAGE_MODEL).trim() || OPENAI_IMAGE_MODEL;
 ...
 */
async function runImageProviders({ prompt, aspectRatio, imageSize, referenceImages, requestedModel }) {
  let lastErr = 'image_generation_failed';
  let hadProvider = false;
  const requested = String(requestedModel || '').trim();
  const wantGemini = /gemini|nano.?banana|flash-image/i.test(requested);
  const wantOpenAi = /gpt-image|dall-e/i.test(requested);
  ...
  const tryOpenAi = async (model) => {
    ...
    hadProvider = true;
    const gen = await callOpenAiImage({
      apiKey: openAiKey,
      prompt,
      aspectRatio,
      imageSize,
    ...
    console.warn(`[image-gen] OpenAI ${gen.model || model} failed (${lastErr})`);
    return null;
  };
  ...
  const tryGemini = async (model) => {
    ...
    hadProvider = true;
    for (const id of pickModels(model || process.env.NANO_BANANA_MODEL)) {
  ...
  if (wantGemini) {
    const hit = await tryGemini(requested);
    if (hit) return hit;
  } else if (wantOpenAi) {
    const hit = await tryOpenAi(requested);
    if (hit) return hit;
  }

  const openAiHit = await tryOpenAi(OPENAI_IMAGE_MODEL);
  if (openAiHit) return openAiHit;
  console.warn(`[image-gen] OpenAI failed (${lastErr}) — falling back to Gemini`);
  const geminiHit = await tryGemini();
  if (geminiHit) return geminiHit;
...
/**
 * Generate an image (OpenAI GPT Image 2, Gemini fallback), meter the Usage
 * Balance, persist the result to storage. `referenceImages` (user
...
export async function generateChatImage({
  ...
  if (typeof authorizeUsage === 'function') {
    const auth = await authorizeUsage({ actionType: 'image_gen' });
    if (!auth?.ok) {
      return {
        ok: false,
        ...
        add_funds: true,
        hint: 'The user needs to add funds to their Usage Balance before this image can be generated.',
      };
    }
  ...
  // Fixed-raw-cost action: settle the full reserved amount.
  await settle({
    ...
    try {
      await logUsage({
        ...
        metadata: {
          ...
          provider: gen.provider || 'google',
          usage_reservation_id: reservation?.id || null,
        },
      });
    } catch {
