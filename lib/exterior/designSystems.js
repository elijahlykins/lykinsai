...
//
// COLOR: LLMs revert to purple/indigo gradients on every build if left to
// their own taste. So the accent is NOT baked into the briefs — the server
// rotates through ACCENT_PALETTES (deliberately purple-free) and injects the
// chosen tokens into the [DESIGN_SYSTEM] block, with an explicit no-purple
// rule. Purple only happens when the USER asks for it.
//
...
// Curated accent palettes the server rotates through per build. Each carries
// everything a brief needs: solid tokens plus Tailwind tint/text/ring
// classes. The gradient recipe is OPT-IN ONLY — offered to the model solely
// for when the user explicitly asks for a gradient. No purples — if the
// user wants purple they'll say so, and the model may honor that.
...
/** Rotate accents so consecutive builds don't all land on the same color. */
export function pickAccentPalette() {
 ...
 * one fixed look.
 */
export function pickDesignSystem(userMessage) {
  const text = String(userMessage || '');
  if (!text.trim()) return DEFAULT_SYSTEM_ID;
  for (const id of MATCH_PRIORITY) {
    if (DESIGN_SYSTEMS[id].keywords.test(text)) return id;
  }
  for (const f of FORMAT_POOLS) {
    if (f.re.test(text)) return rollFrom(f.pool);
  }
  return rollFrom(DEFAULT_ROTATION);
}

/**
 * Format the [DESIGN_SYSTEM] prompt block for a coded-artifact turn: the
 * chosen brief, this build's rotated ACCENT tokens (with the no-purple rule),
 * a complexity-scaling rule, and a one-line catalog so the model can switch
 * systems when the user asks for a different look mid-conversation.
 */
export function formatDesignSystemBlock(systemId, accent = pickAccentPalette()) {
