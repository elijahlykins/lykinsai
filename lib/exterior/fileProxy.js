...
/** Secret used to sign download tokens. Falls back to the service-role key,
 * which is always present wherever artifacts are persisted. */
function proxySecret() {
  const secret =
    process.env.FILE_PROXY_SECRET || process.env.SUPABASE_SERVICE_ROLE_KEY || '';
  if (!secret) throw new Error('file_proxy_secret_missing');
  return secret;
}
