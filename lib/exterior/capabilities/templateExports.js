...
// Named typefaces for template restyles (decks/docs). Style-only edits pass
// `font` without re-sending sections so slide copy stays byte-identical.
const FONT_STACKS = {
  inter: { family: '"Inter", ui-sans-serif, system-ui, sans-serif', href: 'https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700&display=swap' },
  system: { family: 'ui-sans-serif, system-ui, -apple-system, "Segoe UI", Roboto, Helvetica, Arial, sans-serif', href: null },
  georgia: { family: 'Georgia, "Times New Roman", Times, serif', href: null },
  times: { family: '"Times New Roman", Times, Georgia, serif', href: null },
  playfair: { family: '"Playfair Display", Georgia, serif', href: 'https://fonts.googleapis.com/css2?family=Playfair+Display:wght@500;700&display=swap' },
  'space-grotesk': { family: '"Space Grotesk", ui-sans-serif, system-ui, sans-serif', href: 'https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;600;700&display=swap' },
  spacegrotesk: { family: '"Space Grotesk", ui-sans-serif, system-ui, sans-serif', href: 'https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;600;700&display=swap' },
  merriweather: { family: 'Merriweather, Georgia, serif', href: 'https://fonts.googleapis.com/css2?family=Merriweather:wght@400;700&display=swap' },
  mono: { family: 'ui-monospace, "JetBrains Mono", Menlo, Consolas, monospace', href: 'https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;600&display=swap' },
  jetbrains: { family: '"JetBrains Mono", ui-monospace, Menlo, monospace', href: 'https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;600&display=swap' },
};
...
export function resolveFont(font) {
  const key = String(font || '')
    ...
    .replace(/[\s_]+/g, '-');
  if (!key) return null;
  if (FONT_STACKS[key]) return { key, ...FONT_STACKS[key] };
  // Allow a raw CSS font-family string (quoted names only — no url()).
  if (/^[a-z0-9\s,"'-]+$/i.test(String(font)) && !/\burl\s*\(/i.test(String(font))) {
    return { key: 'custom', family: String(font).trim(), href: null };
  }
  return FONT_STACKS.inter;
}

function fontOverrideCss(fontSpec) {
  if (!fontSpec?.family) return '';
  return `\n    body, .doc, .slide { font-family: ${fontSpec.family} !important; }`;
}

function fontHeadLink(fontSpec) {
  if (!fontSpec?.href) return '';
  return `\n  <link rel="preconnect" href="https://fonts.googleapis.com" />\n  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />\n  <link href="${fontSpec.href}" rel="stylesheet" />`;
}
 ...
 */
export function buildDocumentHtml(title, sections, { templateType = 'document', theme = null, font = null } = {}) {
  ...
  const fontSpec = resolveFont(font);
...
<head>
  ...
  <title>${escapeHtml(title)}</title>${fontHeadLink(fontSpec)}
  <style>${BASE_STYLES}${accentOverrideCss(resolveAccent(theme))}${fontOverrideCss(fontSpec)}
...
export function buildSlideshowHtml(title, sections, { templateType = 'slideshow', theme = null, font = null } = {}) {
  const items = Array.isArray(sections) ? sections : [];
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>${escapeHtml(title)}</title>${fontHeadLink(fontSpec)}
  <style>${BASE_STYLES}${accentOverrideCss(accent)}${fontOverrideCss(fontSpec)}
