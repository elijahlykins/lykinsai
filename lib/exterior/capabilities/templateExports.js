...
function markdownToPdfBlocks(raw) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Table → one block per row, pipes flattened to spaced columns.
    if (trimmed.includes('|') && i + 1 < lines.length && isTableSeparator(lines[i + 1])) {
      const header = splitTableRow(trimmed);
      blocks.push({ kind: 'table-head', text: header.map((c) => stripInline(c)).join('   ') });
      i++; // consume separator
      while (
        i + 1 < lines.length &&
        lines[i + 1].includes('|') &&
        ...
        blocks.push({ kind: 'table-row', text: row.map((c) => stripInline(c)).join('   ') });
        i++;
      }
      continue;
    }
      ...
      for (const block of markdownToPdfBlocks(sec.body)) {
        switch (block.kind) {
          ...
          case 'code':
            ...
            break;
          case 'table-head':
            write(block.text, { size: 10, style: 'bold', gapBefore: 4, gapAfter: 2 });
            break;
          case 'table-row':
            write(block.text, { size: 10, gapAfter: 2 });
            break;
          default:
