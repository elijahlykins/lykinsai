...
import { isCodeExecDisabled, runPythonSnippet } from '../runPython.js';
...
// denylists — NOT a real sandbox. Production defaults to DISABLED
// (LYKN_ENABLE_CODE_EXEC must opt in). A proper isolate
// (gVisor/Firecracker/worker with no secrets in env) is the long-term fix.
...
export async function runCode(args = {}) {
  if (isCodeExecDisabled()) {
    return {
      ...
      hint: 'Server-side code execution is disabled (set LYKN_ENABLE_CODE_EXEC=true to opt in). Return code in your reply instead.',
    };
  }
