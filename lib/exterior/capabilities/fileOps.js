...
export async function manageFile(args = {}, ctx = {}) {
  ...
  const activeContent =
    typeof ctx.activeArtifactContent === 'string' && ctx.activeArtifactContent.trim()
      ? ctx.activeArtifactContent
      : typeof ctx.activeArtifactCode === 'string' && ctx.activeArtifactCode.trim()
        ? ctx.activeArtifactCode
        : '';
  let editedInPlace = false;

  // Targeted find/replace against the open file artifact (HTML/md/csv/…).
  if (edits.length > 0 && (action === 'create' || action === 'edit') && !(wantsFullRewrite && content.trim())) {
    if (!activeContent) {
      return {
        ok: false,
        error: 'no_artifact_to_edit',
        hint:
          '`edits` only works while an existing file artifact is open. ' +
          'Pass full `content` for a new file, or open the artifact and retry.',
      };
    }
    const patched = applyArtifactEdits(activeContent, edits);
    if (!patched.ok) return patched;
    content = patched.code;
    editedInPlace = true;
  }

  // Open file + no full_rewrite ⇒ patches only (same discipline as React/templates).
  if (
    ...
    return {
      ...
      hint:
        'A file artifact is already open. Call again with `edits` ONLY — {find, replace} patches ' +
        'copied EXACTLY from the current source in [ARTIFACT_OPEN]. Do not resubmit full `content` ' +
        'for a targeted change. ONLY if the user asked to rebuild the whole file, retry with ' +
        'complete `content` plus `full_rewrite: true`.',
    };
  }
  ...
  if (action === 'create' || action === 'edit') {
    if (!content.trim() && !editedInPlace) {
      return { ok: false, error: 'content_required' };
    }
    let result = {
      ok: true,
      action: editedInPlace ? 'edit' : action,
      filename,
      format: sourceFormat,
      content,
      char_count: content.length,
      edits_applied: editedInPlace ? edits.length : undefined,
      // Echo source so the client can keep refining after a patch turn.
