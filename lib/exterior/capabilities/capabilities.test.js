...
import { capabilityCtx, mimeTypeForFilename } from '../capabilityStorage.js';
...
test('buildReactArtifact multi-file project bundles and edits by path', async () => {
  ...
  const opsOk = await buildReactArtifact(
    {
      title: 'Game',
      file_ops: [
        {
          op: 'write',
          path: 'game/Slash.js',
          content: 'export default function Slash(){ return <div>/</div>; }\n',
        },
      ],
      edits: [{ path: 'App.jsx', find: 'name="A"', replace: 'name="slash"' }],
    },
    { activeArtifactFiles: created.artifact_files, activeArtifactEntry: 'App.jsx' },
  );
  assert.equal(opsOk.ok, true);
  assert.equal(opsOk.file_count, 3);
  assert.ok(opsOk.artifact_files.some((f) => f.path === 'game/Slash.js'));
});

test('capabilityCtx keeps multi-file source so Build edits do not ask for files again', async () => {
  const files = [
    {
      path: 'App.jsx',
      content:
    ...
    },
    {
      ...
      content: 'export default function Editor(){ return <textarea />; }\n',
    },
  ];

  const promoted = await buildReactArtifact(
    {
      title: 'Notes',
      file_ops: [
        {
          op: 'write',
          path: 'SlashMenu.jsx',
          content: 'export default function SlashMenu(){ return <div>/</div>; }\n',
        },
      ],
    },
    capabilityCtx({
      activeArtifactCode: 'export default function App(){ return <div>notes</div>; }\n',
    }),
  );
  assert.equal(promoted.ok, true, promoted.hint || promoted.error);
  assert.ok(promoted.artifact_files.some((f) => f.path === 'SlashMenu.jsx'));
  assert.ok(promoted.artifact_files.some((f) => f.path === 'App.jsx'));

  const kept = await buildReactArtifact(
    {
      title: 'Notes',
      file_ops: [
        {
          op: 'write',
          path: 'components/SlashMenu.jsx',
          content: 'export default function SlashMenu(){ return <div>/</div>; }\n',
        },
      ],
      edits: [
        {
          path: 'components/Editor.jsx',
          find: 'return <textarea />;',
          replace: 'return <><textarea /><SlashMenu /></>;',
        },
      ],
    },
    capabilityCtx({
      activeArtifactCode: files[0].content,
      activeArtifactFiles: files,
      activeArtifactEntry: 'App.jsx',
    }),
  );
  assert.equal(kept.ok, true, kept.hint || kept.error);
  assert.equal(kept.file_count, 3);
  const editor = kept.artifact_files.find((f) => f.path === 'components/Editor.jsx');
  assert.match(editor.content, /SlashMenu/);
});
...
test('buildReactArtifact requires edits when artifact is open', async () => {
  ...
  // A dump of `code` on an edit turn is ignored — patches still apply.
  const dumpIgnored = await buildReactArtifact(
    {
      title: 'App',
      code: 'export default function App(){ return <div>rewritten</div>; }',
      edits: [{ find: 'Hello', replace: 'Patched' }],
    },
    { activeArtifactCode: base },
  );
  assert.equal(dumpIgnored.ok, true);
  assert.match(dumpIgnored.artifact_code || '', /Patched/);
  assert.doesNotMatch(dumpIgnored.artifact_code || '', /rewritten/);
});

test('buildReactArtifact rejects syntax errors before preview', async () => {
  const broken = await buildReactArtifact({
    title: 'Broken',
    code: 'export default function App(){ return <div>oops; }',
  });
  assert.equal(broken.ok, false);
  assert.equal(broken.error, 'compile_error');
  assert.equal(broken.preview_html, undefined);

  const base = 'export default function App(){ return <div>hi</div>; }';
  const badEdit = await buildReactArtifact(
    { title: 'X', edits: [{ find: '<div>hi</div>', replace: '<div>hi' }] },
    { activeArtifactCode: base },
  );
  assert.equal(badEdit.ok, false);
  assert.equal(badEdit.error, 'compile_error');
});

test('react runner never paints a red stack dump', async () => {
  const ok = await buildReactArtifact({
    title: 'Ok',
    code: 'export default function App(){ return <div>hi</div>; }',
  });
  assert.equal(ok.ok, true);
  assert.match(ok.preview_html, /Still putting it together/);
  assert.doesNotMatch(ok.preview_html, /errEl\.style\.display = "block"/);
  const readyAt = ok.preview_html.indexOf('type: "ready"');
  const renderAt = ok.preview_html.indexOf('root.render');
  assert.ok(readyAt > renderAt);
  assert.match(
    ok.preview_html.slice(renderAt),
    /root\.render[\s\S]*hideBoot\(\);\s*reportToParent\(\{ type: "ready"/,
  );
});
...
test('buildTemplate accepts full sections when allowFullRewrite even without flag', async () => {
  const active = [
    { id: 's1', heading: 'One', body: 'Alpha' },
    { id: 's2', heading: 'Two', body: 'Beta' },
  ];
  const ok = await buildTemplate(
    {
      ...
      theme: 'blue',
      sections: [
        { id: 's1', heading: 'Problem', body: 'New copy' },
        { id: 's2', heading: 'Market', body: 'Stats' },
        { id: 's3', heading: 'Sources', body: 'Links' },
      ],
    },
    { activeArtifactSections: active, allowFullRewrite: true },
  );
  assert.equal(ok.ok, true);
  assert.equal(ok.sections.length, 3);
  assert.equal(ok.sections[0].body, 'New copy');
  assert.equal(ok.title, 'LYKN Glass');
});
