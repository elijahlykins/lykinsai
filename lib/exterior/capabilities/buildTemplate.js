...
export async function buildTemplate(args = {}, ctx = {}) {
  ...
  // Authorized redesign: prefer a full `content`/`sections` dump over patches.
  const shippingFullBody =
    allowFullRewrite &&
    ((sections.length > 0 && !sectionEdits.length) ||
      (contentArg.trim() && !contentEdits.length));
  const wantsFullRewrite =
    allowFullRewrite && (args.full_rewrite === true || shippingFullBody);
  ...
  // HARD RULE — open template + no authorized full rewrite ⇒ style-only,
  // section_edits, or content_edits. When allowFullRewrite is set (style
  // rematch / redesign), accept a full sections dump even if the model
  // forgot full_rewrite: true — that flag miss was causing phantom builds.
  if (
    activeSections.length > 0 &&
    !allowFullRewrite &&
    !patchedInPlace &&
