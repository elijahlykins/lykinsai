...
export async function buildTemplate(args = {}, ctx = {}) {
  ...
  // HARD RULE — open template + no full_rewrite ⇒ style-only, section_edits,
  // or content_edits. Reject full sections/content resubmits that silently
  ...
  if (
    ...
    return {
      ...
      hint:
        ...
        'ONLY if the user explicitly asked to rebuild/restyle the whole thing, retry with complete ' +
        '`sections` plus `full_rewrite: true`.',
    };
  }
