...
export async function buildTemplate(args = {}, ctx = {}) {
  ...
  const title = stripEmoji(String(args.title || '').trim()) || 'Untitled';
  ...
  const previewHtml =
    templateType === 'email'
      ...
      : isSlideLike
        ? buildSlideshowHtml(title, normalizedSections, { templateType, theme })
        : buildDocumentHtml(title, normalizedSections, { templateType, theme });
