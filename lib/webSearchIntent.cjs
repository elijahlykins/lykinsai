...
// Search TOOLS are attached on every real (non-greeting) turn. The model
// reads the prompt and decides whether to call them. Keyword lists do not
// gate tool availability.
// Serper pre-fetch (shouldForceWebSearch) is only a latency shortcut when
// the user already opted into Web / Deep research, or the turn is already
// a live/current ask. Missing a pre-fetch must not hide the tools.
...
function isPhaticTurn(text) {
  const t = String(text || '').trim();
  if (!t) return true;
  return GREETING_FOR_SEARCH.test(t);
}
 ...
 * Serper so a stale model doesn't invent a table from memory.
 * Pre-fetch only. Search tools are attached on every real turn so the
 * model can decide whether to look something up.
 */
...
/** Explicit opt-in OR live-freshness — latency pre-fetch only, not tool gating. */
...
/**
 * Attach live web tools on every real turn. Greetings stay lean.
 * The model chooses whether to search from the prompt. Do not match
 * topic keywords here — that cannot cover news, lookups, or follow-ups.
 */
function messageWantsWebTools(text) {
  const t = String(text || '').trim();
  if (!t) return false;
  if (isPhaticTurn(t)) return false;
  return true;
}
