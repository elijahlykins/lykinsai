...
export async function resolveVaultAttachment(ctx, opts = {}) {
  ...
  if (storagePath) {
    ...
    // Ephemeral transit objects (`<uid>/ephemeral/…`, e.g. generated 3D GLBs)
    // are deleted by the reconciler sweep after ~24h — referencing that path
    // would hand the vault a card whose bytes are about to vanish. Saving to
    // the vault means the user is KEEPING it, so copy the bytes into durable
    // storage instead. If the copy fails, fall through to the reference (the
    // link still works until the sweep) rather than losing the save outright.
    if (/^[^/]+\/ephemeral\//i.test(gate.path)) {
      try {
        const { data: blob, error: dlErr } = await ctx.supabaseAdmin.storage
          .from(bucket)
          .download(gate.path);
        if (!dlErr && blob) {
          const buf = Buffer.from(await blob.arrayBuffer());
          if (buf.length) {
            const resolvedMime = mimeType || mimeTypeForFilename(String(filename || gate.path));
            const name = deriveFilename(filename, resolvedMime, gate.path, title);
            ...
            if (stored.ok) {
              return {
                type: inferAttachmentKind(stored.mime_type || resolvedMime, name, gate.path),
                url: stored.file_url,
                name,
      ...
      } catch { /* fall through to the (time-limited) reference */ }
    }
