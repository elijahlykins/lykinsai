...
test('real turns get search tools; greetings do not', () => {
  assert.equal(messageWantsWebTools('hello'), false);
  assert.equal(messageWantsWebTools('thanks'), false);
  assert.equal(messageWantsWebTools("what's on the news"), true);
  assert.equal(messageWantsWebTools('is clive a word'), true);
  assert.equal(messageWantsWebTools('would it be valid in scrabble'), true);
  assert.equal(messageWantsWebTools('chatgpt said yes haha'), true);
  assert.equal(messageWantsWebTools('explain how transformers work'), true);
  assert.equal(messageWantsWebTools('rewrite this paragraph more clearly'), true);
});

test('pre-fetch is a shortcut, not the tool gate', () => {
  assert.equal(shouldForceWebSearch("what's on the news"), false);
  assert.equal(shouldForceWebSearch('is clive a word'), false);
  assert.equal(shouldForceWebSearch('search online for Terra pricing'), true);
  assert.equal(shouldForceWebSearch("what's today's news on OpenAI"), true);
});
