...
const { looksLikeWorkInNamedApp } = require("./agentWorkDestination.cjs");

/**
 * "Go there and write it" is one job — but only when that is the whole ask.
 *
 * A chain that names a place and then keeps going ("go into pinterest, find
 * blue presentation inspo, based off one of those build me a presentation") is
 * several jobs that happen to start at a destination, and collapsing it strands
 * everything after the first clause.
 */
function isOneJobInANamedApp(text) {
  const t = String(text || "").trim();
  if (!looksLikeWorkInNamedApp(t)) return false;
  if (/\bthen\b|\bafter that\b|\bonce (?:you|that)\b|\bbased off\b|\bbased on\b/i.test(t)) {
    return false;
  }
...
function splitCompoundSkillStep(step) {
  ...
  // Work in a named app stays one step — going there and writing there is
  // one job, and splitting it strands the writing with nowhere to land.
  if (isOneJobInANamedApp(raw)) return [raw];
...
function splitMultiStepPrompt(text) {
  ...
  // Don't peel "open <app> and write…" into open + search — that's one job.
  if (openSearch && !looksLikeWorkInNamedApp(raw)) {
...
function cleanPlanStep(step) {
  ...
  // "Take a new note in notion and write…" runs as one step — rewriting the
  // verbs loses the destination the user named.
  if (isOneJobInANamedApp(s)) return s;
