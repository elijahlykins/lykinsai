...
import { morphologicalVariants } from './queryExpansion.js';
import { chunkTextForSynthesis } from '../../synthesis-service.js';

test('morphologicalVariants: plural/singular pairs', () => {
  const v = morphologicalVariants('prosthetics');
  assert.ok(v.includes('prosthetic'));
  const v2 = morphologicalVariants('company');
  assert.ok(v2.includes('companies'));
});
