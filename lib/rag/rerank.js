 ...
 */
export function rerankProvider() {
  if (process.env.COHERE_API_KEY) return 'cohere';
  if (process.env.VOYAGE_API_KEY) return 'voyage';
  if (process.env.OPENAI_API_KEY && process.env.RAG_LLM_RERANK === '1') return 'llm';
  return 'none';
}
 ...
 */
export async function rerankCandidates(query, candidates, { topN, timeoutMs = 8000 } = {}) {
