...
export async function runSubModelDelegate({
  ...
  const system = [
    ...
    '',
    '[DELEGATED_TASK]',
    `You are "${model.name}", a sub-agent reporting back to the user's main agent.`,
    'Complete ONLY the assigned task. Be thorough but concise.',
    'Return a structured report the main agent can forward — no meta commentary about being a sub-agent.',
  ]
    .filter(Boolean)
    .join('\n')
    .trim();
