...
import { resolveInferenceTarget } from '../inference/resolveGateway.js';
...
export async function runSubModelDelegate({
  ...
  const target = resolveInferenceTarget(DELEGATE_MODEL);
  const apiKey = String(
    (target.keyVar && process.env[target.keyVar]) || openaiApiKey || process.env.OPENAI_API_KEY || '',
  ).trim();
  if (!apiKey) {
    return { ok: false, error: 'delegation_unavailable', message: 'No inference API key configured.' };
  }
  ...
  try {
    loop = await runAgentLoop({
      provider: target.gateway === 'openrouter' ? 'openrouter' : 'openai',
      model: target.upstreamId || DELEGATE_MODEL,
      systemPrompt: system,
      userContent: userMsg,
