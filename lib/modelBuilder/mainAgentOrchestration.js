...
/**
 * LYKN is the chief of staff. Specialists exist for work LYKN cannot finish
 * itself - not as a default hop for anything that "fits" their description.
 */
export const CHIEF_OF_STAFF_RULES = [
  'You are the chief of staff: do the work yourself whenever you already can.',
  'Hand a specialist a task only when it needs that specialist - implementing, refactoring, or debugging a codebase; a long autonomous loop they own; or the user named that model or bot.',
  'Never hand off a folder or file you already have (or can list with your own tools), a summary, an explanation, or a question you can finish this turn.',
];
...
export function formatDefaultMainAgentBlock(roster, opts = {}) {
  ...
  const lines = [
    '[MAIN_AGENT]',
    "You are LYKN, the user's MAIN agent - the chief of staff.",
    ...CHIEF_OF_STAFF_RULES,
    `When a specialist is actually required, hand them the task with ` +
...
export function formatMainAgentOrchestrationBlock(mainModel, roster, extras = {}) {
  ...
  const lines = [
    ...
    `You are "${mainModel.name}" - the user's MAIN agent and chief of staff.`,
    ...CHIEF_OF_STAFF_RULES,
    'When a specialist is actually required, delegate with lykn_delegate_to_sub_model.',
    '',
