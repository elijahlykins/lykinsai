...
const SELECT_COLS =
  'id, user_id, name, slug, kind, base_url, description, auth_type, auth_header_name, auth_query_param, default_headers, allow_writes, status, last_used_at, use_count, last_error, created_at, updated_at';
...
function sanitizeAuthType(raw) {
  ...
  if (!['none', 'bearer', 'header', 'query'].includes(s)) {
    throw new CustomConnectionError("auth_type must be one of: none, bearer, header, query");
  }
  return s;
}
...
export async function callApp({ client, userId, connection: ref, method = 'GET', path = '', query = null, body = null, headers = null } = {}) {
  ...
  if (!conn) {
    return { ok: false, error: 'connection_not_found', message: `No custom connection "${ref}". Call lykn_list_apps to see what's connected.` };
  }
  if (conn.status !== 'active') {
    return { ok: false, error: 'connection_paused', message: `The "${conn.slug}" connection is paused.` };
  }
  ...
  try {
    const init = { method: m, headers: reqHeaders, signal: controller.signal, redirect: 'follow' };
