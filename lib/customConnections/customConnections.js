// ============================================================================
// lib/customConnections/customConnections.js — LEGACY bring-your-own-API-key
// ============================================================================
// Isolated / legacy REST dispatcher. The future universal external-tool lane
// is MCP (lib/mcp). kind: 'mcp' remains unused here on purpose.
...
export async function createCustomConnection(client, userId, payload = {}) {
  ...
  const { encryptToken } = await import('../security/credentialStore.js');
...
export async function callApp({ client, userId, connection: ref, method = 'GET', path = '', query = null, body = null, headers = null } = {}) {
  ...
  if (conn.status !== 'active') {
    return {
      ...
      message: `The "${conn.slug}" connection is paused.`,
    };
  }
  ...
  if (conn.auth_type !== 'none' && conn.secret_encrypted) {
    let secret = '';
    try {
      const { decryptToken } = await import('../security/credentialStore.js');
