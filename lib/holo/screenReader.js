  ...
  properties: {
    ...
    nextStep: {
      ...
      properties: {
        action: {
          type: 'string',
          enum: ['click', 'type', 'scroll', 'press_key', 'navigate', 'go_back', 'hover', 'wait', 'done'],
        },
        target: {
          type: 'string',
        ...
        key: {
          type: 'string',
          description: 'For press_key: the key name (Enter, Escape, Tab, ArrowDown, PageDown…).',
        },
        url: {
          type: 'string',
          description: 'For navigate: the absolute URL to open (only URLs visible on screen / known from the goal — never invented).',
        },
        scrollDirection: {
          type: 'string',
          enum: ['up', 'down'],
          description: 'For scroll: which direction to scroll.',
        },
...
export function openAiOutputTokenParam(model, n) {
  const m = String(model || '').toLowerCase();
  const tokens = Math.max(1, Math.floor(Number(n) || 1));
  if (/^gpt-[56]|^o[1-9]|^o4-/.test(m)) {
    return { max_completion_tokens: tokens };
  }
  return { max_tokens: tokens };
}

/** GPT-5+ / o-series only allow default temperature — omit the field. */
export function openAiSamplingParams(model, temperature = 0.1) {
  const m = String(model || '').toLowerCase();
  if (/^gpt-[56]|^o[1-9]|^o4-/.test(m)) {
    return {};
  }
...
export function formatScreenBriefForHolo(brief) {
  ...
  if (brief.nextStep) {
    ...
    if (ns.key) parts.push(`- Key: ${ns.key}`);
    if (ns.url) parts.push(`- URL: ${ns.url}`);
    if (ns.scrollDirection) parts.push(`- Scroll: ${ns.scrollDirection}`);
...
export async function runScreenReader({
  ...
  const res = await fetch(OPENAI_API, {
    ...
    body: JSON.stringify({
      model: readerModel,
      ...openAiSamplingParams(readerModel, 0.1),
      ...openAiOutputTokenParam(readerModel, isPreview ? 1600 : 1400),
      // Routing hint only — see the note in lib/agentModelProviders.js. This
      // path is the weakest candidate of the lot: the system prompt below is
      // the sole stable prefix and sits near OpenAI's ~1024-token floor, so it
      // may cache nothing. Harmless if so.
      ...
      messages: [
        {
          ...
          content:
            ...
            '- OTHER ACTIONS: "scroll" (with scrollDirection) when the target is below/above the fold or you need to read ' +
            'more of a list/page; "press_key" (with key) — Escape to dismiss a blocking popup/menu, Enter to submit a focused field, ' +
            'ArrowDown/PageDown for keyboard-driven UIs; "navigate" (with url) to jump straight to a URL that is visible on screen ' +
            'or given in the goal — never invent URLs; "go_back" when the last click was a wrong turn or you need to return to ' +
            'the results/list page; "hover" (with clickPoint) to reveal hover-only row actions, menus, or tooltips; ' +
            '"wait" when the page is clearly still loading (spinner, skeleton, blank content).\n' +
