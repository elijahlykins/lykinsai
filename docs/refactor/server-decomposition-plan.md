...
Wave 4 (2026-08-26) extracted three moderate live domains, 21 routes, zero manifest drift: `account.routes.js` (7 routes: preferences GET/PATCH, night-shift briefs, steward items GET/POST/PATCH, DELETE /api/account; `USER_PREFERENCE_DEFAULTS`/`sanitisePreferencesPatch`/`NIGHT_SHIFT_BRIEF_MAX_AGE_MS` moved as domain-private; `stripe` + `invalidateUserModelCache` passed; `revokeAppleToken` direct-imported from `lib/appleAuth.js`), `storage.routes.js` (4 routes: ai/vault-search, storage/signed-url, storage/file-proxy-url, vault/backfill-descriptions; `SIGNED_URL_ALLOWED_BUCKETS` + the description-backfill helper trio moved as domain-private; `SIGNED_URL_TTL_SECONDS` stays in server.js because the files router shares it and is passed to both), `assist.routes.js` (10 routes: imagine-image, describe-image, transcribe, meeting-chunk, summarize-conversation, clean-transcript, live-assist, meeting-notes, suggest, name-grid; no module-level mutable state — every cache in the band was already per-request).
Storage deps `replaceSynthesisChunks`/`createSynthesisUserClient` (passed) and `chunkTextForSynthesis` (direct import) are LEGACY CANDIDATES — pending Memory Architecture Replacement; they were passed/imported unchanged, not modularized further.
The learning/user-model band (learned, user-facts, beliefs, rules, applied, ai/feedback, synthesis activity/revisions — 20 routes) was inventoried and deliberately left inline as a MEMORY-REPLACEMENT CANDIDATE; discover (2 routes, ~1,300 helper lines, reads the synthesis profile) was also deferred.
`server.js`: 21,123 → 19,395 lines.
Wave 5 (2026-08-26) extracted two sensitive non-chat boundaries, 17 routes, zero manifest drift: `authFlows.routes.js` (6 routes as two registrars because the bands register at two positions: `registerAppleAuthRoutes` for the SIWA token exchange, `registerEmailAuthRoutes` for signup-start/resend/verify + password-reset-start/confirm; the email handler factories moved into the once-run registrar closure), `platform.routes.js` (3 routes as three registrars: metrics ingest, feedback, project invite), `connectionsOAuth.routes.js` (8 routes, one registrar: OAuth start, `/oauth/callback/:provider` popup HTML, connect-info, connect-token, connections list/sync/patch/delete — still the last routes registered before the global error handler).
`resendClient` and `findAuthUserByEmail` stay in server.js because feedback, email-auth, and project-invite all share them (single Resend client identity, passed to both modules); `pickUserDisplayName` and `invalidateConnectedToolsCache` are shared server.js helpers passed as deps; connectors-service functions are direct imports (OAuth state is DB-backed — no in-process state moved); the `/oauth/` CSP relaxation is path-based in bootstrap and unaffected.
`server.js`: 19,395 → 18,554 lines.
Wave 6 (2026-08-26) extracted the billing route boundary, 9 routes, zero manifest drift: `billing.routes.js` (one registrar: billing/me, checkout, credits, topup, stripe-config, trial-checkout, portal, waitlist GET+POST).
The Stripe webhook was NOT moved (its raw-body position above the JSON parser is load-bearing — pre-limiter wave), and ALL shared billing infrastructure stayed in server.js and is passed via deps: `resolveUserPlan`, `requireAppAccess`, `handleStripeEvent`/`syncSubscriptionToBilling`, `loadBillingRow`, the `userPlanCache`/`appAccessGrace`/`freeCreditsCache` Maps, price maps, and the comped-email shortcut (`isCompedProEmail`/`COMPED_PRO_PLAN_ID` — deferred finding, preserved).
`PLAN_LIMITS`/`creditPackById` (frontend-owned `src/lib/pricing-config.js`) are passed as deps so that import edge stays in server.js only; `getCreditWallet`/`listCreditTopups`/`getUserMonthlyUsage` are stateless direct imports.
Billing characterization tests were added to `serverCriticalRoutes.test.mjs` (unauthenticated 401 contract for all 8 non-public billing routes + stripe-config public shape).
`server.js`: 18,554 → 18,114 lines.
Wave 7 (2026-08-26) extracted the five pre-limiter platform routes, zero manifest drift, as FOUR registrars across two modules so every route stays at its exact bootstrap position: `stripeWebhook.routes.js` (`registerStripeWebhook`, called between CORS and the branching JSON parser — `express.raw` raw-body signature verification preserved; `stripe` + hoisted `handleStripeEvent` passed by identity, billing infra NOT moved) and `preLimiterPlatform.routes.js` (`registerClientErrorRoute` + `registerHealthRoute` called right after the JSON parser and before the auth core, `registerFileProxyAndArtifactRoutes` called after `requireAuth`/`supabaseAdmin` exist but before the global `/api/` limiter).
Because `/api/health` registers before the `supabaseAdmin` const is initialized, its registrar receives a lazy getter (`getSupabaseAdmin: () => supabaseAdmin`) — the handler resolves the client per request, exactly as the inline code's late module-binding lookup did.
Deferred findings preserved verbatim: the artifacts-rebuild limiter exemption (route stays pre-limiter) and the inert 10kb client-error parser (global 1mb parser still consumes the body first; oversized bodies still 400 via zod, never 413).
`verifyFileToken`/`FILE_PROXY_ROUTE`/`mimeTypeForFilename`/`buildReactArtifact` became direct imports in the module and were pruned from server.js; `isArtifactsHost` stays (bootstrap artifacts-host gate).
A limiter-exemption characterization test for the rebuild route was added to `serverCriticalRoutes.test.mjs`; `serverMiddlewareOrder.test.mjs` continues to pin rawParser-first, webhook-before-JSON-parser, and the exact 5-route pre-limiter set.
