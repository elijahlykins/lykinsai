...
Waves 1–4 deliberately left in place: `authLimiter`-gated authFlows (incl. SIWA token exchange + metrics ingest + feedback + project invite), all services, all pollers, all global middleware, AI/chat streaming, billing/Stripe, OAuth infrastructure, learning/user-model routes, discover, and the five pre-limiter platform routes.
