...
Remaining inline after Wave 6 (33 routes): the five pre-limiter platform routes (stripe webhook, client-error, health, `/f/:token`, artifacts rebuild), chat core ×5 (`/api/ai/models`, stream-guest, invoke, local-tool-result, stream), the learning/user-model band ×21 (MEMORY-REPLACEMENT CANDIDATE), and discover ×2 (legacy-adjacent). Billing routes are fully extracted; all services, pollers, shared billing infra, and global middleware stay in bootstrap.
