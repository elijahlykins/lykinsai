...
| Legacy adaptive browse loop inside `electron/ownedBrowserAct.cjs` | Only runs when `LYKN_BROWSER_AGENT=legacy` or the model endpoint is unavailable; the modular `browser-agent/` is the default. Removal = behavior change (fallback disappears). Keep until the fallback is formally retired. |
