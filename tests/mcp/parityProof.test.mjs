...
test('MCP failure never falls back to connectors, Vault mirror, or lykn_call_app', async () => {
  ...
  const streamSrc = fs.readFileSync(path.join(ROOT, 'server.js'), 'utf8');
...
test('legacy Vault-sync connectors remain temporarily and stay off the MCP runtime', () => {
  assert.equal(fs.existsSync(path.join(ROOT, 'connectors-service.js')), true);
  const mcpRuntime = [
    'lib/mcp/executeMcpTool.js',
    'lib/mcp/mcpConnectionManager.js',
    'lib/mcp/chatTurn.js',
  ...
  for (const rel of mcpRuntime) {
    const src = fs.readFileSync(path.join(ROOT, rel), 'utf8');
    assert.equal(src.includes('connectors-service'), false, `${rel} must not import vault-sync connectors`);
    assert.equal(src.includes('CONNECTOR_REGISTRY'), false, rel);
    assert.equal(src.includes('runSync('), false, rel);
  }
});
