...
test("finish returns a review draft and persists nothing until explicit save", async (t) => {
  const { directory, service } = serviceFixture(t);
  service.start({ name: "Demo", objective: "File an issue" });
  service.record({
    kind: "mcp",
    action: "create_issue",
...
test("autonomous task observation requires the explicitly selected source Task", (t) => {
  ...
  service.start({ sourceTaskId: "task_source" });
...
test("explicit source Task capture normalizes local, MCP, remote, and handoff boundaries", async (t) => {
  const { service } = serviceFixture(t);
  service.start({ sourceTaskId: "task_source" });
  const base = { taskId: "task_source", type: "progress" };
  ...
  service.recordTaskEvent({
    ...base,
    association: { remoteTargetId: "staging" },
...
test("recovered targets require explicit apply and create a new version", async (t) => {
  ...
  assert.equal(service.listWorkflows()[0].recoveredUpdate.id, proposal.id);
  ...
  assert.equal(service.listWorkflows()[0].recoveredUpdate, undefined);
});

test("workflow edits rebuild parameter paths and reject stale concurrent saves", async (t) => {
  const { service } = serviceFixture(t);
  service.start({ name: "Edit" });
  service.record({
    kind: "browser",
    action: "fill",
